...
public class AsetVoltlClassCorlSynchronizer extends AsetVoltlClassBase implements DataSynchronizer {
    ...
    public AsetVoltlClassCorlSynchronizer(NamedParameterJdbcTemplate namedParameterJdbcTemplate, MongoDatabase mongodb) {
    ...
    public void sync(Set<Map<String, Object>> keySet) {
        log.info("Updating {} aset_voltl_class_corl ...", keySet.size());
        sync();
    }
    ...
    public void delete(Set<String> rowidSet) {
        sync();
        log.info("{} {} item has been deleted", rowidSet.size(), CollectionName.aset_voltl_class_corl);
    }

    private void sync() {
        Date lastSyncTime = new Date();
        Map<Map<String, Object>, List<Document>> groupedMap = loadAndGroupDocuments("ASET_VOLTL_CLASS_CORL");
