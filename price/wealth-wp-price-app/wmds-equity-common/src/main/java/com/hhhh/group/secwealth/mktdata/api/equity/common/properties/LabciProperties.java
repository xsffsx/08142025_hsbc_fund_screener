...
@Getter
@Setter
public class LabciProperties {

	private static final Logger logger = LoggerFactory.getLogger(LabciProperties.class);
	...
	private String tradingDayUrl;

	private String tradingParamPrefix;
	...
	private void initLabciFieldMapper() {
		if (!this.fieldMapper.containsKey(LabciProperties.DEFAULT_SITE)) {
			LabciProperties.logger.error(
		...
		for (Map.Entry<String, Map<String, Map<String, Map<String, String>>>> siteEntry : this.fieldMapper.entrySet()) {
			...
			if (!siteMapper.containsKey(LabciProperties.DEFAULT_PRODUCT)) {
				LabciProperties.logger.error(
			...
			for (Map.Entry<String, Map<String, Map<String, String>>> productEntry : siteMapper.entrySet()) {
				...
				if (!productMapper.containsKey(LabciProperties.DEFAULT_EXCHANGE)) {
					LabciProperties.logger.error(
	...
	public String getLabciService(final String site, final boolean delay) {
		...
		} else if (this.service.containsKey(LabciProperties.DEFAULT_SITE)) {
			serviceOptions = this.service.get(LabciProperties.DEFAULT_SITE);
		} else {
			LabciProperties.logger
					.error("Please check your configuration: \"labci.service\", default site should be \"DEFAULT\"");
			throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
		}
		String key = delay ? LabciProperties.DELAY_OPTION : LabciProperties.REALTIME_OPTION;
	...
	public String getLabciClosure(final String site) {
		...
		} else if (this.closure.containsKey(LabciProperties.DEFAULT_SITE)) {
			result = this.closure.get(LabciProperties.DEFAULT_SITE);
		} else {
			LabciProperties.logger
	...
	public String getLabciTokenAppCode(final String site) {
		...
		} else if (this.tokenAppCode.containsKey(LabciProperties.DEFAULT_SITE)) {
			result = this.tokenAppCode.get(LabciProperties.DEFAULT_SITE);
		} else {
			LabciProperties.logger.error(
	...
	public String getLabciTokenTimezone(final String site) {
		...
		} else if (this.tokenTimezone.containsKey(LabciProperties.DEFAULT_SITE)) {
			result = this.tokenTimezone.get(LabciProperties.DEFAULT_SITE);
		} else {
			LabciProperties.logger.error(
	...
	public String getLabciNbUrl() {
		if (StringUtil.isInValid(this.nbUrl)) {
			LabciProperties.logger
					.error("Please check your configuration: \"labci.nb-url\", the returned value is invalid");
			throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
		}
		return this.nbUrl;
	}
	...
	public String getExchangeMapping(String exchange) {
		...
		return this.exchangeMapping.get(LabciProperties.DEFAULT_EXCHANGE);
	}
