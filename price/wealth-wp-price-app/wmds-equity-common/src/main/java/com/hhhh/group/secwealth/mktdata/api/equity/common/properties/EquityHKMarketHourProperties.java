...
@Getter
@Setter
public class EquityHKMarketHourProperties {

	private static final Logger logger = LoggerFactory.getLogger(EquityHKMarketHourProperties.class);
	...
	public boolean isMarketOpen(final String marketTypeCode, final DelayProdInfo... delayProd) {
		try {
			if (StringUtil.isInValid(marketTypeCode) || this.sessionInfo.get(marketTypeCode) == null) {
				EquityHKMarketHourProperties.logger.info("The marketTypeCode is empty...");
				return false;
			} else {
			...
			String checkDateStr = DateUtil.formatToString(EquityHKMarketHourProperties.CHECK_DATE, currentTime,
					this.timezoneStr);
			...
			if (isWeekend) {
				EquityHKMarketHourProperties.logger.info("{} is weekend...", currentDateStr);
				return false;
			}
			boolean isHoliday = this.isHoliday(yearStr, currentDateStr, currentTimeStr, checkDateStr);
			if (isHoliday) {
				EquityHKMarketHourProperties.logger.info("{} is holiday...", currentDateStr);
				return false;
			}
			...
			if (isTradHour) {
				EquityHKMarketHourProperties.logger.debug("{} is in trading hour...", currentTimeStr);
				return true;
			}
			if (delayProd.length > 0 && delayProd[0] != null && StringUtil.isValid(delayProd[0].getAsetClassCde())
				...
				if (isDelayed) {
					EquityHKMarketHourProperties.logger.info("{} is in delay trading hour , delay ProdCode : {}",
							currentTimeStr, delayProd[0].getProdCode());
					return true;
				}
			}
		} catch (Exception e) {
			EquityHKMarketHourProperties.logger.info("isMarketOpen has some error :" + e);
			return false;
		}
		return false;
	}
	...
	private boolean isTradHour(String weekStr, String currentDateStr, String currentTimeStr,String marketDelayCloseStr) {
		...
		if (this.sessionInfo.get(this.marketTypeCode) != null) {
			...
			EquityHKMarketHourProperties.logger.info("The tradingHour is empty...");
			return false;
		}
		...
		for (String tradHour : tradHours) {
			...
			closeTimeMap.put(EquityHKMarketHourProperties.OPEN_TIME, tH[0]);
			closeTimeMap.put(EquityHKMarketHourProperties.CLOSE_TIME, tH[1]);
		...
		for (Map<String, String> closeTimeMap : exchCloseTime) {
			boolean isTradHour = this.checkTime(currentTimeStr,
					currentDateStr + " " + closeTimeMap.get(EquityHKMarketHourProperties.OPEN_TIME),
					currentDateStr + " " + closeTimeMap.get(EquityHKMarketHourProperties.CLOSE_TIME), this.timezoneStr,Integer.valueOf(marketDelayCloseStr)*60*1000);
			if (isTradHour) {
				EquityHKMarketHourProperties.logger.debug("{} is in trading hour...", currentTimeStr);
				return true;
			}
	...
	private boolean isSpecial(String yearStr, String currentDateStr, String currentTimeStr, String checkDataStr) {
		if (this.special.get(this.marketTypeCode) != null
			...
			if (specials.containsKey(checkDataStr)) {
				...
				if (StringUtil.isValid(special.get(EquityHKMarketHourProperties.TRADING_HOUR))) {
					String[] spec = special.get(EquityHKMarketHourProperties.TRADING_HOUR).split(SymbolConstant.SYMBOL_COMMA);
					for (String sD : spec) {
						...
						if (isSpecial) {
							EquityHKMarketHourProperties.logger.info(
									"{} is holiday , holiday description : {} , but is trading hour now",
									currentDateStr, special.get(EquityHKMarketHourProperties.DESC));
							return false;
						}
					}
				} else {
					EquityHKMarketHourProperties.logger.info("{} is holiday , holiday description : {}", currentDateStr,
							special.get(EquityHKMarketHourProperties.DESC));
					return true;
				}
			}
		}
	...
	private boolean isHoliday(String yearStr, String currentDateStr, String currentTimeStr, String checkDataStr) {
		if (this.holiday.get(this.marketTypeCode) != null
			...
			if (holidays.containsKey(checkDataStr)) {
				...
				if (StringUtil.isValid(holiday.get(EquityHKMarketHourProperties.TRADING_HOUR))) {
					String[] holi = holiday.get(EquityHKMarketHourProperties.TRADING_HOUR).split(SymbolConstant.SYMBOL_COMMA);
					for (String hD : holi) {
						...
						if (isHoliday) {
							EquityHKMarketHourProperties.logger.info(
									"{} is holiday , holiday description : {} , but is trading hour now",
									currentDateStr, holiday.get(EquityHKMarketHourProperties.DESC));
							return false;
						}
					}
				} else {
					EquityHKMarketHourProperties.logger.info("{} is holiday , holiday description : {}", currentDateStr,
							holiday.get(EquityHKMarketHourProperties.DESC));
					return true;
				}
			}
		}
	...
	private boolean checkTime(final String currentTime, final String openTime, final String closeTime,
		...
		try {
			...
			EquityHKMarketHourProperties.logger.error("Conver the current time error....");
			EquityHKMarketHourProperties.logger.error("currentTime : {} , openTime : {} , closeTime : {} , timeZone : {}",
