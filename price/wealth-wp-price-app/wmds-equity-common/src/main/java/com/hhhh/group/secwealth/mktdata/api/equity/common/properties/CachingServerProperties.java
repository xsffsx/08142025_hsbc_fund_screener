...
@Component
@ConfigurationProperties(prefix = "caching-server")
@ConditionalOnProperty(value = "service.quotes.hkEquity.injectEnabled", havingValue = "true")
@Getter
@Setter
public class CachingServerProperties {

    private static final Logger logger = LoggerFactory.getLogger(CachingServerProperties.class);
    ...
    private String suspendFlagConfig;

    private String labciUrl;
    ...
    private Map<String, String> responseStatus = new HashMap<>();
    ...
    private void initLabciFieldMapper() {
        if (!this.fieldMapper.containsKey(CachingServerProperties.DEFAULT_SITE)) {
            CachingServerProperties.logger.error(
                "Please check your configuration \"caching-server.field-mapper.{site}\", default site should be \"DEFAULT\"");
        ...
        for (Map.Entry<String, Map<String, Map<String, Map<String, String>>>> siteEntry : this.fieldMapper.entrySet()) {
            ...
            if (!siteMapper.containsKey(CachingServerProperties.DEFAULT_PRODUCT)) {
                CachingServerProperties.logger.error(
                    "Please check your configuration \"caching-server.field-mapper.{site}.{product}\", default product should be \"DEFAULT\"");
                throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
            }
            for (Map.Entry<String, Map<String, Map<String, String>>> productEntry : siteMapper.entrySet()) {
                ...
                if (!productMapper.containsKey(CachingServerProperties.DEFAULT_EXCHANGE)) {
                    CachingServerProperties.logger.error(
                        "Please check your configuration \"caching-server.field-mapper.{site}.{product}.{exchange}\", default exchange should be \"DEFAULT\"");
                    throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
                }
                for (Map.Entry<String, Map<String, String>> exchangeEntry : productMapper.entrySet()) {
                    ...
                    if (exchangeMapper.isEmpty()) {
                        CachingServerProperties.logger.error(
                            "Please check your configuration \"caching-server.field-mapper.{site}.{product}.{exchange}.Map\", Map should not be empty");
                        throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
                    }
    ...
    public String getLabciService(final String site, final boolean delay) {
        ...
        } else if (this.service.containsKey(CachingServerProperties.DEFAULT_SITE)) {
            serviceOptions = this.service.get(CachingServerProperties.DEFAULT_SITE);
        } else {
            CachingServerProperties.logger
                .error("Please check your configuration: \"caching-server.service\", default site should be \"DEFAULT\"");
            throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
        }
        String key = delay ? CachingServerProperties.DELAY_OPTION : CachingServerProperties.REALTIME_OPTION;
        if (serviceOptions.containsKey(key)) {
            ...
            if (StringUtil.isValid(result)) {
                ...
                CachingServerProperties.logger
                    .error("Please check your configuration: \"caching-server.service\", the returned value is invalid");
                throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
            }
        } else {
            CachingServerProperties.logger
                .error("Please check your configuration: \"caching-server.service\", options are \"realtime\" or \"delay\"");
            throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
        }
    }
    ...
    public String getLabciClosure(final String site) {
        ...
        } else if (this.closure.containsKey(CachingServerProperties.DEFAULT_SITE)) {
            result = this.closure.get(CachingServerProperties.DEFAULT_SITE);
        } else {
            CachingServerProperties.logger
                .error("Please check your configuration: \"caching-server.closure\", default site should be \"DEFAULT\"");
            throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
        }
        if (StringUtil.isInValid(result)) {
            CachingServerProperties.logger
                .error("Please check your configuration: \"caching-server.closure\", the returned value is invalid");
            throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
        }
        return result;
    }
    ...
    public String getLabciTokenAppCode(final String site) {
        ...
        } else if (this.tokenAppCode.containsKey(CachingServerProperties.DEFAULT_SITE)) {
            result = this.tokenAppCode.get(CachingServerProperties.DEFAULT_SITE);
        } else {
            CachingServerProperties.logger
                .error("Please check your configuration: \"caching-server.token-app-code\", default site should be \"DEFAULT\"");
            throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
        }
        if (StringUtil.isInValid(result)) {
            CachingServerProperties.logger
                .error("Please check your configuration: \"caching-server.token-app-code\", the returned value is invalid");
            throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
        }
        return result;
    }
    ...
    public String getLabciTokenTimezone(final String site) {
        ...
        } else if (this.tokenTimezone.containsKey(CachingServerProperties.DEFAULT_SITE)) {
            result = this.tokenTimezone.get(CachingServerProperties.DEFAULT_SITE);
        } else {
            CachingServerProperties.logger
                .error("Please check your configuration: \"caching-server.token-timezone\", default site should be \"DEFAULT\"");
            throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
        }
        if (StringUtil.isInValid(result)) {
            CachingServerProperties.logger
                .error("Please check your configuration: \"caching-server.token-timezone\", the returned value is invalid");
            throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
        }
        return result;
    }
     ...
     *
     * <p>
     * <b> Get the corresponding Labci fields with the given fields and keys.
     * </b>
     * </p>
     *
     * @param fields
     ...
     *
     * <p>
     * <b> Get the corresponding Labci fields with the given fields, defaultKey
     * and keys. </b>
     * </p>
     *
     * @param fields
    ...
    private List<String> getLabciFields(final List<String> fields, final String defaultKey, final String[] keys) {
        ...
        Map<String, Map<String, String[]>> fieldMappers =
            (Map<String, Map<String, String[]>>) ArgsHolder.getArgs(Constant.THREAD_INVISIBLE_RESPONSE_LABCI_FIELD_MAPPER);
     ...
     * intersect with labciFieldMapperKeys. </b>
     * </p>
     *
     * @param defaultKey
     * @param keys
    ...
    private String[] getLabciField(final List<Map<String, String>> labciFieldMappers, final String field) {
        ...
        if (StringUtil.isInValid(result)) {
            CachingServerProperties.logger
                .error("Please check your configuration: \"caching-server.field-mapper\", can't find field: " + field);
            throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
        }
    ...
    public String getResponseStatus(final int statusCode) {
        if (StringUtil.isValid(this.responseStatus.get(String.valueOf(statusCode)))) {
            return this.responseStatus.get(String.valueOf(statusCode));
        }
        return null;
    }

    public String getExchangeMapping(final String exchange) {
