...
@ConditionalOnProperty(value = "service.quotes.hkEquity.injectEnabled")
@Getter
@Setter
public class EquityHKLabciProperties {

	private static final Logger logger = LoggerFactory.getLogger(EquityHKLabciProperties.class);
	...
	private String midfsBmpUrl;

	private String midfsListUrl;

	private String midfsDetailUrl;
	...
	private void initLabciFieldMapper() {
		if (!this.fieldMapper.containsKey(EquityHKLabciProperties.DEFAULT_SITE)) {
			EquityHKLabciProperties.logger.error(
		...
		for (Map.Entry<String, Map<String, Map<String, Map<String, String>>>> siteEntry : this.fieldMapper.entrySet()) {
			...
			if (!siteMapper.containsKey(EquityHKLabciProperties.DEFAULT_PRODUCT)) {
				EquityHKLabciProperties.logger.error(
			...
			for (Map.Entry<String, Map<String, Map<String, String>>> productEntry : siteMapper.entrySet()) {
				...
				if (!productMapper.containsKey(EquityHKLabciProperties.DEFAULT_EXCHANGE)) {
					EquityHKLabciProperties.logger.error(
	...
	public String getLabciService(final String site, final boolean delay) {
		...
		} else if (this.service.containsKey(EquityHKLabciProperties.DEFAULT_SITE)) {
			serviceOptions = this.service.get(EquityHKLabciProperties.DEFAULT_SITE);
		} else {
			EquityHKLabciProperties.logger
					.error("Please check your configuration: \"labci.service\", default site should be \"DEFAULT\"");
			throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
		}
		String key = delay ? EquityHKLabciProperties.DELAY_OPTION : EquityHKLabciProperties.REALTIME_OPTION;
	...
	public String getLabciClosure(final String site) {
		...
		} else if (this.closure.containsKey(EquityHKLabciProperties.DEFAULT_SITE)) {
			result = this.closure.get(EquityHKLabciProperties.DEFAULT_SITE);
		} else {
			EquityHKLabciProperties.logger
	...
	public String getLabciTokenAppCode(final String site) {
		...
		} else if (this.tokenAppCode.containsKey(EquityHKLabciProperties.DEFAULT_SITE)) {
			result = this.tokenAppCode.get(EquityHKLabciProperties.DEFAULT_SITE);
		} else {
			EquityHKLabciProperties.logger.error(
	...
	public String getLabciTokenTimezone(final String site) {
		...
		} else if (this.tokenTimezone.containsKey(EquityHKLabciProperties.DEFAULT_SITE)) {
			result = this.tokenTimezone.get(EquityHKLabciProperties.DEFAULT_SITE);
		} else {
			EquityHKLabciProperties.logger.error(
	 ...
	 *
	 * <p>
	 * <b> Access Labci URL. </b>
	 * </p>
	 *
	 * @return
	 */
	public String getLabciUrl() {
		if (StringUtil.isInValid(this.labciUrl)) {
			EquityHKLabciProperties.logger
					.error("Please check your configuration: \"labci.url\", the returned value is invalid");
			throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_LABCI_ILLEGAL_CONFIGURATION);
		}
		return this.labciUrl;
	}
	

	public String getMidfsBmpQuoteUrl() {
		if (StringUtil.isInValid(this.midfsBmpUrl)) {
			EquityHKLabciProperties.logger
					.error("Please check your configuration: \"midfs.bmpQuoteUrl\", the returned value is invalid");
			throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_MIDFS_ILLEGAL_CONFIGURATION);
		}
		return this.midfsBmpUrl;
	}

	public String getMidfsQuoteListUrl() {
		if (StringUtil.isInValid(this.midfsListUrl)) {
			EquityHKLabciProperties.logger
					.error("Please check your configuration: \"midfs.quoteListUrl\", the returned value is invalid");
			throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_MIDFS_ILLEGAL_CONFIGURATION);
		}
		return this.midfsListUrl;
	}

	public String getMidfsQuoteDetailUrl() {
		if (StringUtil.isInValid(this.midfsDetailUrl)) {
			EquityHKLabciProperties.logger
					.error("Please check your configuration: \"midfs.quoteDetailUrl\", the returned value is invalid");
			throw new IllegalConfigurationException(ExCodeConstant.EX_CODE_MIDFS_ILLEGAL_CONFIGURATION);
		}
		return this.midfsDetailUrl;
	}
	...
	public String getExchangeMapping(String exchange) {
		...
		return this.exchangeMapping.get(EquityHKLabciProperties.DEFAULT_EXCHANGE);
	}
