...
import com.hhhh.group.secwealth.mktdata.api.equity.index.dao.QuoteUserDao;
		...
		if (!StringUtils.isEmpty(quoteUser.getUserExtnlId())) {
			...
			if (quoUser == null) {
				...
				try {
					...
					QuoteUserForIndex quoUser2 = this.quoteUserDao.findOne(example);
					if (quoUser2 == null) {
				...
				List<QuoteUserForIndex> quoteUserForIndexs = this.quoteUserDao.findAll(example);
				if (quoteUserForIndexs.size() > 1) {
					for (QuoteUserForIndex quoteUserForIndex : quoteUserForIndexs) {
						if (quoteUserForIndex.getUserReferenceId() <= quoUser.getUserReferenceId()) {
							quoUser = quoteUserForIndex;
							QuoteUserService.logger
									.debug("User change...and user id is " + quoteUserForIndex.getUserReferenceId());
						} else {
							quoteUserForIndex.setMonitorFlag((long) 2);
							this.quoteUserDao.saveAndFlush(quoteUserForIndex);
							QuoteUserService.logger
									.debug("User save twice should be update monitorflag 2...and user id is "
											+ quoteUserForIndex.getUserReferenceId());
						}
					}
				}
			}
