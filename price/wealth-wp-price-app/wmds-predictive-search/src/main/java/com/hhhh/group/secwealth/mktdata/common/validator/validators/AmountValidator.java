        ...
        for (String field : this.fieldArray) {
            ...
            if (null != value && !(value instanceof JSONNull) && StringUtil.isValid(value.toString().trim())) {
                try {
                    ...
                    LogUtil.error(AmountValidator.class, e.getMessage(), e);
                    validatorList.add(new ValidatorError(ErrTypeConstants.AMOUNT_INVALID, field,
                        "The parameter " + field + " is not an amount.", this.getClass().getSimpleName()));
                }
            }
        }
