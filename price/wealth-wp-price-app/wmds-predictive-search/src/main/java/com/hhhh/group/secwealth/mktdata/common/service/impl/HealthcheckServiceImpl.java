...
public class HealthcheckServiceImpl implements HealthcheckService {
    ...
    public void init() throws FileNotFoundException {
        // URL keystoreFileUrl =
        // this.getClass().getClassLoader().getResource(this.keystoreFile);
        // URL passwordKeystoreFileUrl =
        // this.getClass().getClassLoader().getResource(this.passwordKeystoreFile);
        try {
            MICipher cipbher = new MICipher(this.keystoreFile, this.passwordKeystoreFile, this.ivParam);
            this.decryptedUserid = cipbher.decrypt(this.userid);
            this.decryptedPassword = cipbher.decrypt(this.password);
            HealthcheckServiceImpl.healthCheckProperties
                .load(this.getClass().getClassLoader().getResourceAsStream(HealthcheckServiceImpl.FILE_PATH));
        } catch (Exception e) {
            LogUtil.error(HealthcheckServiceImpl.class,
                "Can't init HealthcheckServiceImpl|exception=" + e.getMessage() + "|keystoreFile=" + this.keystoreFile
                    + "|passwordKeystoreFile=" + this.passwordKeystoreFile + "|ivParam=" + this.ivParam + "|userid=" + this.userid
    ...
    public boolean authenticate(final String userid, final String password) {
        LogUtil.debug(HealthcheckServiceImpl.class, "dashboardLogin: userid: {}, password: {}", userid, password);
    ...
    public void healthDashboardApi(final StringBuilder sb,
        final String site/* , final String samlString */) {
        // StringBuilder sb = new StringBuilder();
            ...
            if (venders.length > 0) {
                for (String vender : venders) {
                    ...
                    if (vender.contains("wpc_data")) {
                       ...
                       * else if (vender.contains("mstr")) { if
                       * (vender.contains("globalApi")) {
                       * mstrHealthCheckMsg(sb, vender, site, this.mstrGlobaUrl
                       * + CommonConstants.SYMBOL_SLASH +
                       * MstarConstants.MSTARID); } else if
                       * (vender.contains("cnApi") &&
                       * StringUtil.isValid(this.mstrCnUrl)) {
                       * mstrHealthCheckMsg(sb, vender, site, this.mstrCnUrl);
                       * } } else if (vender.contains("db")) {
                       * dbHealthCheckMsg(sb, vender, site); } else if
                       * (vender.contains("wpc_webservice")) {
                       * wpcWebServiceHealthCheckMsg(sb, vender, site,
                       * samlString); }
    ...
    private void wpcDataHealthCheckMsg(final StringBuilder sb, final String vender, final String site) {
        ...
        try {
            ...
            LogUtil.error(HealthcheckServiceImpl.class, "health check fail: " + e.getMessage(), e);
    ...
    /*
     * private void mstrHealthCheckMsg(final StringBuilder sb, final String
     * vender, final String site, final String url) { // String url = //
     * HealthcheckServiceImpl.healthCheckProperties.getProperty(site+vender+
     * ".url"); String params =
     * HealthcheckServiceImpl.healthCheckProperties.getProperty(site + vender +
     * ".params"); params = params == null ? "" : params; long startTime =
     * System.currentTimeMillis(); try {
     * LogUtil.infoOutboundMsg(HealthcheckServiceImpl.class, VendorType.MSTAR,
     * url, params); String respStr = this.connManager.sendRequest(url,
     * params); long endTime = System.currentTimeMillis(); if
     * (StringUtil.isValid(respStr)) { String time = Long.toString(endTime -
     * startTime); sb.append(HealthcheckServiceImpl.TD_START +
     * HealthcheckServiceImpl.HTML_FONT_START + time +
     * HealthcheckServiceImpl.HTML_FONT_END + HealthcheckServiceImpl.TD_END); }
     * else { sb.append(HealthcheckServiceImpl.TD_START +
     * HealthcheckServiceImpl.HTML_FONT_ERROR_START +
     * HealthcheckServiceImpl.INVALID_RESPONSE +
     * HealthcheckServiceImpl.HTML_FONT_ERROR_END +
     * HealthcheckServiceImpl.TD_END); } } catch (Exception e) {
     * LogUtil.debug(HealthcheckServiceImpl.class, "health check fail: " +
     * e.getMessage()); sb.append(HealthcheckServiceImpl.TD_START +
     * HealthcheckServiceImpl.HTML_FONT_ERROR_START + e.getMessage() +
     * HealthcheckServiceImpl.HTML_FONT_ERROR_END +
     * HealthcheckServiceImpl.TD_END); } }
     */
    ...
    /*
     * private void dbHealthCheckMsg(final StringBuilder sb, final String
     * vender, final String site) { StringBuilder hql = new StringBuilder();
     * String sql =
     * HealthcheckServiceImpl.healthCheckProperties.getProperty(site + vender +
     * ".sql"); hql.append(sql); long startTime = System.currentTimeMillis();
     * try { EntityManager entityManager = this.baseDao.getEntityManager();
     * Query query = entityManager.createNativeQuery(hql.toString()); Object
     * object = query.getSingleResult();
     * LogUtil.warn(HealthcheckServiceImpl.class, "dbHealthCheckMsg: " +
     * object); long endTime = System.currentTimeMillis(); String time =
     * Long.toString(endTime - startTime);
     * sb.append(HealthcheckServiceImpl.TD_START +
     * HealthcheckServiceImpl.HTML_FONT_START + time +
     * HealthcheckServiceImpl.HTML_FONT_END + HealthcheckServiceImpl.TD_END); }
     * catch (Exception e) { LogUtil.debug(HealthcheckServiceImpl.class,
     * "health check fail: " + e.getMessage());
     * sb.append(HealthcheckServiceImpl.TD_START +
     * HealthcheckServiceImpl.HTML_FONT_ERROR_START + e.getMessage() +
     * HealthcheckServiceImpl.HTML_FONT_ERROR_END +
     * HealthcheckServiceImpl.TD_END); } }
    ...
    /*
     * private void wpcWebServiceHealthCheckMsg(final StringBuilder sb, final
     * String vender, final String site, final String samlString) { String
     * function = HealthcheckServiceImpl.healthCheckProperties.getProperty(site
     * + vender + ".function"); long startTime = System.currentTimeMillis();
     * try { Object obj = doServiceForHealthCheck(null, function, null, site,
     * samlString); long endTime = System.currentTimeMillis(); String time =
     * Long.toString(endTime - startTime);
     * sb.append(HealthcheckServiceImpl.TD_START +
     * HealthcheckServiceImpl.HTML_FONT_START + time +
     * HealthcheckServiceImpl.HTML_FONT_END + HealthcheckServiceImpl.TD_END); }
