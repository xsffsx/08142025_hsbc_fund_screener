        ...
        for (String field : this.fieldArray) {
            ...
            if (StringUtil.isValid(value.toString())) {
                try {
                    ...
                    LogUtil.error(IntegerValidator.class, e.getMessage(), e);
                    validatorList.add(new ValidatorError(ErrTypeConstants.INTEGER_INVALID, field,
                        "The parameter " + field + " is not an integer.", this.getClass().getSimpleName()));
                }
            } else {
                json.put(field, null);
            }
