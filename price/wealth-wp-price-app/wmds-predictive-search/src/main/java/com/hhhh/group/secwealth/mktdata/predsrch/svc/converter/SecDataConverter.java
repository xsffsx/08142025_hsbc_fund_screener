...
import com.hhhh.group.secwealth.mktdata.predsrch.dal.model.stock.StockInstm;
import com.hhhh.group.secwealth.mktdata.predsrch.dal.model.stock.StockInstmLst;
import com.hhhh.group.secwealth.mktdata.predsrch.svc.beans.DataSiteEntity;

public class SecDataConverter implements DataConverter {
    ...
    public void handleData(final File file, final RefDataLst refDataList, final DataSiteEntity entity, final boolean skipErrRecord)
        ...
        if (null != lst && null != lst.getStockInstm()) {
            LogUtil.debug(SecDataConverter.class, "Start to handle data for the file=" + file.getName());
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (errors[0] && skipErrRecord) {
                    LogUtil.error(SecDataConverter.class,
                    ...
                    so.setExchange_analyzed(obj.getStockInstmSeg().getMktProdTrdCde());
                    // from refData
            ...
            LogUtil.debug(SecDataConverter.class, "End to handle data for the file=" + file.getName());
        }
    }
