...
public class QueryExpressionHandler {
	...
	public static String[] getSortingFieldsBySteps(final int step) {
		...
		switch (step) {
		case 1:
			sortingFields = new String[] { "countryTradableCodeWeight", "productTypeWeight", "productName" };
			break;
		case 2:
			sortingFields = new String[] { "countryTradableCodeWeight", "productTypeWeight", "productName" };
			break;
		case 3:
			sortingFields = new String[] { "countryTradableCodeWeight", "productTypeWeight", "symbol" };
			break;
		case 4:
			sortingFields = new String[] { "countryTradableCodeWeight", "productTypeWeight", "symbol" };
			break;
	...
	public static void constructContainsQueryExpression(final String searchField, final String keyWord,
		...
		searchClauses.append(CommonConstants.SYMBOL_LEFT_BRACKET).append(searchField)
				.append(QueryExpressionHandler.EQUAL_SEARCH).append(CommonConstants.SYMBOL_ASTERISK)
				.append(toQueryString(keyWord)).append(CommonConstants.SYMBOL_ASTERISK)
	...
	public static void constructStartWithQueryExpression(final String searchField, final String keyWord,
			final StringBuffer searchClauses) {
		searchClauses.append(CommonConstants.SYMBOL_LEFT_BRACKET).append(searchField)
				.append(QueryExpressionHandler.EQUAL_SEARCH).append(toQueryString(keyWord))
				.append(CommonConstants.SYMBOL_ASTERISK).append(CommonConstants.SYMBOL_RIGHT_BRACKET);
	}
	...
	public static String buildCommonQueryExpression(final String queryType, final String[] assetClasses,
			final String searchField, final String keyWord, final List<SearchCriteria> filters) throws Exception {
		...
		if (null != filters && filters.size() > 0) {
			...
			// "(symbol_analyzed:*u622*) AND (NOT resChannelCde_analyzed:srbpi) AND
			// (productType_analyzed:UT)"
			...
			searchClauses.append(getFilterQuery(filters.get(0).getCriteriaKey(), filters.get(0).getCriteriaValue(),
					filters.get(0).getOperator()));
			for (int j = 1; j < size; j++) {
				searchClauses.append(CriteriaOperator.AND).append(getFilterQuery(filters.get(j).getCriteriaKey(),
						filters.get(j).getCriteriaValue(), filters.get(j).getOperator()));
			}
			if (filters.size() != 1 && !(filters.get(0).getOperator().equalsIgnoreCase(CriteriaOperator.NE_OPERATOR))) {
	 ...
	 *             the exception
	 */
	public static String buildQueryExpression(final String keyWord, final String[] assetClass,
			final String[] searchFields, final List<SearchCriteria> filters,
			final Map<String, String> searchTypeByField) throws Exception {
		...
		if (null != keyWord) {
			flag = 1;
			searchClauses.append(CommonConstants.SYMBOL_LEFT_BRACKET);
			StringBuffer key = new StringBuffer().append(toQueryString(keyWord))
					.append(CommonConstants.SYMBOL_ASTERISK);
			searchClauses.append(CommonConstants.SYMBOL_LEFT_BRACKET).append(searchTypeByField.get(searchFields[0]))
					.append(key).append(CommonConstants.SYMBOL_RIGHT_BRACKET);
			for (int i = 1; i < searchFields.length; i++) {
				searchClauses.append(CriteriaOperator.OR).append(CommonConstants.SYMBOL_LEFT_BRACKET)
						.append(searchTypeByField.get(searchFields[i])).append(key)
						.append(CommonConstants.SYMBOL_RIGHT_BRACKET);
			}
			searchClauses.append(CommonConstants.SYMBOL_RIGHT_BRACKET);
		}
	...
	public static String getFilterQuery(final String key, final String value, final String operator) throws Exception {
		if (operator.equalsIgnoreCase(CommonConstants.SYMBOL_EQUAL)
				|| operator.equalsIgnoreCase(CriteriaOperator.EQ_OPERATOR)) {
			return new StringBuilder(key).append(QueryExpressionHandler.EQUAL_SEARCH).append(toQueryString(value))
					.toString();
			...
			if (key.equalsIgnoreCase(QueryExpressionHandler.PRODUCT_KEY)) {
				...
				return new StringBuilder(CriteriaOperator.NOT).append(CommonConstants.SYMBOL_LEFT_BRACKET)
						.append(getProductByKey(fields[1], fields[2], fields[3]))
						.append(CommonConstants.SYMBOL_RIGHT_BRACKET).toString();
			} else {
			...
			StringBuilder sb = new StringBuilder(CommonConstants.SYMBOL_LEFT_BRACKET).append(keyAppender)
					.append(toQueryString(strs[0]));
	...
	/**
	 * <p>
	 * <b> Return a expression by symbol,countryTradeableCode and productType </b>
	 * </p>
	 * .
	 * 
	 * @param symbol
	 ...
	 */
	public static String getProductByKey(final String symbol, final String countryTradableCode,
			final String productType) {
		return new StringBuilder(QueryExpressionHandler.SYMBOL_ANALYZED).append(toQueryString(symbol))
				.append(CriteriaOperator.AND).append(QueryExpressionHandler.COUNTRYTRADABLECODE_ANALYZED)
				.append(countryTradableCode).append(CriteriaOperator.AND).append(addProductType(productType))
				.toString();
