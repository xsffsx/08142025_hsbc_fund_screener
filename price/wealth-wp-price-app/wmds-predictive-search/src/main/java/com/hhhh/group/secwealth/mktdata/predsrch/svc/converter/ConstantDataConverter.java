...
public class ConstantDataConverter implements DataConverter {
    ...
    public void handleData(final File file, final RefDataLst refDataList, final DataSiteEntity entity, final boolean skipErrRecord)
        ...
        if (null != lst && null != lst.getGnrcProd()) {
            LogUtil.debug(ConstantDataConverter.class, "Start to handle data for the file=" + file.getName());
            ...
            for (int i = 0; i < size; i++) {
                ...
                SearchableObject so =
                    CommonConverter.handleCommonData(infoSeg, obj.getProdKeySeg(), obj.getProdAltNumSeg(), errors);
                ...
                // Set asetClassCde as exchange
                so.setExchange(infoSeg.getAsetClassCde());
                objList.add(so);
                Thread.sleep(this.delayConversion);
            }
            CommonConverter.handleCommonLog(size, symbolErr, codeErr, entity, file.getName());

            LogUtil.debug(ConstantDataConverter.class, "End to handle data for the file=" + file.getName());
        }
    }
