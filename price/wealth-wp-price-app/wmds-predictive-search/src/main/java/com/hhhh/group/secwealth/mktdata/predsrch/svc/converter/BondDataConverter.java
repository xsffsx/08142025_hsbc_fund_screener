...
public class BondDataConverter implements DataConverter {
    ...
    public void handleData(final File file, final RefDataLst refDataList, final DataSiteEntity entity, final boolean skipErrRecord)
        throws Exception {
        DebtInstmLst lst = (DebtInstmLst) this.unmarshaller
            .unmarshal(new InputStreamReader(new FileInputStream(file), CommonConstants.CODING_UTF8));
        if (null != lst && null != lst.getDebtInstm()) {
            LogUtil.debug(BondDataConverter.class, "Start to handle data for the file=" + file.getName());
            List<DebtInstm> instmList = lst.getDebtInstm();
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (errors[0] && skipErrRecord) {
                    LogUtil.error(BondDataConverter.class,
                        file.getName() + "|Skip to save the record without symbol|prodName=" + so.getProductName());
                } else {
                    DebtInstmSeg seg = obj.getDebtInstmSeg();
                    so.setIsrBndNme(seg.getIsrBndNme());
                    so.setIsrBndNme_analyzed(seg.getIsrBndNme());
                    // from refData
            ...
            LogUtil.debug(BondDataConverter.class, "End to handle data for the file=" + file.getName());
        }
    }
