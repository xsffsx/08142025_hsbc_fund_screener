...
import com.hhhh.group.secwealth.mktdata.predsrch.dal.model.sid.Sid;
import com.hhhh.group.secwealth.mktdata.predsrch.dal.model.sid.SidLst;
import com.hhhh.group.secwealth.mktdata.predsrch.svc.beans.DataSiteEntity;

public class SidDataConverter implements DataConverter {
    ...
    public void handleData(final File file, final RefDataLst refDataList, final DataSiteEntity entity, final boolean skipErrRecord)
        throws Exception {
        SidLst lst =
            (SidLst) this.unmarshaller.unmarshal(new InputStreamReader(new FileInputStream(file), CommonConstants.CODING_UTF8));
        if (null != lst && null != lst.getSid()) {
            LogUtil.debug(SidDataConverter.class, "Start to handle data for the file=" + file.getName());
            List<Sid> instmList = lst.getSid();
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (errors[0] && skipErrRecord) {
                    LogUtil.error(SidDataConverter.class,
            ...
            LogUtil.debug(SidDataConverter.class, "End to handle data for the file=" + file.getName());
        }
    }
