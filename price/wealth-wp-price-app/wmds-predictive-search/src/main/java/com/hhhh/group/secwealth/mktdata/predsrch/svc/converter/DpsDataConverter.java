...
import com.hhhh.group.secwealth.mktdata.predsrch.dal.model.dps.Dps;
import com.hhhh.group.secwealth.mktdata.predsrch.dal.model.dps.DpsLst;
...
public class DpsDataConverter implements DataConverter {
    ...
    /**
     * Convert DPS data file to DPS java object list
     */
    public void handleData(final File file, final RefDataLst refDataList, final DataSiteEntity entity, final boolean skipErrRecord)
        throws Exception {
        DpsLst lst =
            (DpsLst) this.unmarshaller.unmarshal(new InputStreamReader(new FileInputStream(file), CommonConstants.CODING_UTF8));
        if (null != lst && null != lst.getDps()) {
            LogUtil.debug(DpsDataConverter.class, "Start to handle data for the file=" + file.getName());
            List<Dps> instmList = lst.getDps();
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (errors[0] && skipErrRecord) {
                    LogUtil.error(DpsDataConverter.class,
            ...
            LogUtil.debug(DpsDataConverter.class, "End to handle data for the file=" + file.getName());
        }
    }
