...
public class IndexDataConverter implements DataConverter {

    private Unmarshaller unmarshaller;

    private long delayConversion;

    private static final String INDEX_EXCHANGE_WPC_DEFINED_CODE = "idxExchg";
    ...
    public void handleData(final File file, final RefDataLst refDataList, final DataSiteEntity entity, final boolean skipErrRecord)
        ...
        if (null != lst && null != lst.getGnrcProd()) {
            LogUtil.debug(IndexDataConverter.class, "Start to handle data for the file=" + file.getName());
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (obj != null) {
                    ...
                    ProdInfoSeg infoSeg = obj.getProdInfoSeg();
                    // Set ProdUserDefExtSeg fieldValue( fieldCde =
                    // idxExchg) as exchange
                    if (null != infoSeg && null != infoSeg.getProdUserDefExtSeg()) {
                        for (ProdUserDefExtSeg prodUserDefExtSeg : infoSeg.getProdUserDefExtSeg()) {
                            if (null != prodUserDefExtSeg
                                && IndexDataConverter.INDEX_EXCHANGE_WPC_DEFINED_CODE.equals(prodUserDefExtSeg.getFieldCde())) {
                                so.setExchange(prodUserDefExtSeg.getFieldValue());
                                break;
                            }
                        }
                    }
            ...
            LogUtil.debug(IndexDataConverter.class, "End to handle data for the file=" + file.getName());
        }
    }
