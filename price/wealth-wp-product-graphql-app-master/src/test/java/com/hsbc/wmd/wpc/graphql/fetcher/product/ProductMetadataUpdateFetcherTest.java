...
public class ProductMetadataUpdateFetcherTest {

    @InjectMocks
    private ProductMetadataUpdateFetcher productMetadataUpdateFetcher;
    @Mock
    ...
    @Before
    public void setUp() {
        productMetadataUpdateFetcher = new ProductMetadataUpdateFetcher(mongoDatabase);
        ReflectionTestUtils.setField(productMetadataUpdateFetcher, "collection", collection);
        Map<String, Object> metadata = new HashMap<>();
        metadata.put("key", "val");
        Mockito.when(environment.getArgument(anyString())).thenReturn("id").thenReturn(metadata);
    ...
    public void testGet_givenDataFetchingEnvironment_returnsDataFetcherResult() throws Exception {
        ...
        DataFetcherResult dataFetcherResult = productMetadataUpdateFetcher.get(environment);
