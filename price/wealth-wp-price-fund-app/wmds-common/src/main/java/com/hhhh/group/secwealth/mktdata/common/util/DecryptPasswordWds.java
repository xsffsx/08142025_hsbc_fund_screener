 ...
 */
public class DecryptPasswordWds {
    ...
    public static String readLine(final BufferedReader br) throws IOException {
        ...
        if (StringUtils.isNotBlank(line) && line.length() >= DecryptPasswordWds.MAX_STR_LEN) {
    ...
    public static String decrypt(final String pubFilePath, final String keyFilePath, final String pwdFilePath) throws Exception {
        if (!Pattern.compile(DecryptPasswordWds.PATHSTRING).matcher(pwdFilePath).matches()
            || !Pattern.compile(DecryptPasswordWds.PATHSTRING).matcher(keyFilePath).matches()) {
        ...
        try {
            ...
            SecretKeyFactory secretkeyfactory = SecretKeyFactory.getInstance(DecryptPasswordWds.ALGORITHM);
