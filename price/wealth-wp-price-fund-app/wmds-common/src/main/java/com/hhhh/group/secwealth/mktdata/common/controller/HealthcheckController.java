...
import com.hhhh.group.secwealth.mktdata.common.filter.e2etrust.SecurityFilterAutoConfiguration;
...
//    @RequestMapping(value = "healthcheck/samlGenerate", method = {RequestMethod.POST, RequestMethod.GET})
//    public ModelAndView samlGenerate(final HttpServletRequest request, final HttpServletResponse response) throws Exception {
//        LogUtil.debug(HealthcheckController.class, "saml Generate");
//        String samlString = (String) request.getAttribute(DefaultHttpRequestAttributeNames.GENERATED_SAML);
//        request.setAttribute("result", "samlGenerate");
//        if(StringUtil.isInvalid(samlString)) {
//            samlString = "E2eTrust at this Env should be closed, try to generate token in other testing Env !";
//        }
//        request.setAttribute("samlString", samlString);
...
//    }
    ...
    public ModelAndView error(final HttpServletRequest request, final HttpServletResponse response) {
        ...
        try {
            ...
            LogUtil.error(HealthcheckController.class, "request error", e);
            result = "failure";
            systemTime = e.getClass().getName();
        }
        request.setAttribute("result", result);
    ...
    @RequestMapping(value = "healthcheck/dashboard", method = {RequestMethod.POST, RequestMethod.GET})
    public ModelAndView dashboard(final HttpServletRequest request, final HttpServletResponse response) throws Exception {
        ...
        //Will not do login Authenticate see dashboard directly
...
//        return dashboardAuthenticate(request, response, samlString);
    ...
    private ModelAndView dashboardAuthenticate(final HttpServletRequest request, final HttpServletResponse response,
    ...
    private ModelAndView dashboardDetail(final HttpServletRequest request, final HttpServletResponse response,
        final String samlString) {
