...
public class BMCAdvisorBySite {
    ...
    public void init() throws Exception {
        ...
        try (InputStream configIn = this.configFileURL.openStream();
             InputStream mappingIn = this.mappingFileURL.openStream()){
    ...
    public void afterThrowing(final JoinPoint joinPoint, final Throwable e) {
        LogUtil.error(BMCAdvisorBySite.class,
            "BMC LOGGER --> afterReturning --> className: {}, method name: {}, error message: {}", joinPoint.getTarget().getClass()
                .getName(), joinPoint.getSignature().getName(), e.getMessage(), e);
    ...
    public void afterReturning(final JoinPoint joinPoint, final Object event) {
        LogUtil.error(BMCAdvisorBySite.class, "BMC LOGGER --> afterReturning --> className: {}, method name: {}", joinPoint
            .getTarget().getClass().getName(), joinPoint.getSignature().getName());
    ...
    private void genBmcLog(final String errorCode, final String errorMsg, final String errTrackerCode) {
        ...
        try {
            if (this.isPrintFiles != null && true == this.isPrintFiles.booleanValue()) {
                ...
                LogUtil.debug(BMCAdvisorBySite.class, "BMC file generation, path is " + this.bmcLogFilePath + ", errMsg is "
                    + errMsg);
            } else {
                LogUtil.error(BMCAdvisorBySite.class, "BMC_LOG=" + errorCode + " " + errorMsg + " [ErrTrackerCode="
                    + errTrackerCode + "]");
            }
        } catch (Exception e) {
            LogUtil.error(BMCAdvisorBySite.class,
