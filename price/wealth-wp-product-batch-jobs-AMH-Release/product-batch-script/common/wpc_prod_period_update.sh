...
#
# wpc_prod_period_update.sh
# 
if [ $# -lt 3 ] || [ $# -gt 6 ] 
 then
    print "usage: wpc_prod_period_update.sh <country code> <organization code> <product type code list>(optional,split by comma)"
...
#	print "  	ELI_DCDC,ELI-RN"
	print " example: wpc_prod_period_update.sh HK dummy D 'UT,SEC,BOND'"
    exit
fi

if [[ $# -lt 3 ]]; then
    print "usage: wpc_prod_period_update.sh <country code> <organization code> <product type code list>"
    print "example: wpc_prod_period_update.sh HK HBAP ALL"
    print "example: wpc_prod_period_update.sh HK HBAP 'UT,SEC,BOND'"
    exit -1
fi
...
log_file="${ctryCde}_${orgnCde}_wpc_prod_period_update_$(date +%Y%m%d).log"
...
echo "$(date +"["%H:%M:%S" "%d"/"%m"/"%Y"]") WPC prod period update job starting...." >> ${log_path}/${log_file}
echo "$(date +"["%H:%M:%S" "%d"/"%m"/"%Y"]") WPC prod period update job starting...." >> ${log_cloud_path}/${log_cloud_file}
...
${JAVA_PATH} -jar ${JAR_FILE_PATH}/product-product-period-update-job-2.0.0-SNAPSHOT.jar ctryRecCde=${ctryCde} grpMembrRecCde=${orgnCde} prodTypeCde=${prodType} --spring.config.location=${springConfig} >> ${log_path}/${temp_log_file} 2>&1
...
if (( $status == 0 ))
then
    echo "$(date +"["%H:%M:%S" "%d"/"%m"/"%Y"]") WPC prod period update job is done successfully." >> ${log_path}/${log_file}
    echo "$(date +"["%H:%M:%S" "%d"/"%m"/"%Y"]") WPC prod period update job is done successfully." >> ${log_cloud_path}/${log_cloud_file}
else
    logger -t root "WPC0001E M AMH TDC wpc_prod_period_update.sh: Problem encountered when invoking WPC prod period update job. Please contact WMD Support."
    echo "WPC0001E M AMH TDC wpc_prod_period_update.sh: Problem encountered when invoking WPC prod period update job. Please contact WMD Support." >> ${log_cloud_path}/${log_cloud_file}
    echo "$(date +"["%H:%M:%S" "%d"/"%m"/"%Y"]") Failure invoke WPC prod period update job." >> ${log_path}/${log_file}
    echo "$(date +"["%H:%M:%S" "%d"/"%m"/"%Y"]") Failure invoke WPC prod period update job." >> ${log_cloud_path}/${log_cloud_file}
fi

#### WPC prod period update -- END ####