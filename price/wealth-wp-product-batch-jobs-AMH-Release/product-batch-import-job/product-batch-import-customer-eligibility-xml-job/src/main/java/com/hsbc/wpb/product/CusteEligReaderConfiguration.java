...
import com.dummy.wpb.product.model.xml.TradingEligibility;
...
@Configuration
public class CusteEligReaderConfiguration {
    ...
    @Bean
    @StepScope
    public ItemStreamReader<ProductStreamItem> custEligXmlFileReader(ProductService productService,
        ...
        marshaller.setClassesToBeBound(TradingEligibility.class);
        StaxEventItemReader<TradingEligibility> staxEventItemReader = new StaxEventItemReader<>();
        staxEventItemReader.setFragmentRootElementName("prodTradElg");
        ...
        return new ProductStreamItemByKeyItemReaderBuilder<TradingEligibility>()
                .name("custEligXmlFileReader")
    ...
    private final Function<TradingEligibility, ProductKey> toProductKey = tradingEligibility -> {
        ProductKeySegment prodKeySeg = tradingEligibility.getProdKeySeg();
