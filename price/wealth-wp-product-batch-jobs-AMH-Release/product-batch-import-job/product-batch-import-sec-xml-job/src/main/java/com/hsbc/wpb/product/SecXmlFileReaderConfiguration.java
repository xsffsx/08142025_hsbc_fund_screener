...
import com.dummy.wpb.product.model.xml.StockInstrument;
...
@Configuration
public class SecXmlFileReaderConfiguration {
    ...
    @Bean
    @StepScope
    public ItemStreamReader<ProductStreamItem> secXmlFileReader(ProductService productService,
        ...
        marshaller.setClassesToBeBound(StockInstrument.class);
        StaxEventItemReader<StockInstrument> staxEventItemReader = new StaxEventItemReader<>();
        staxEventItemReader.setFragmentRootElementName("stockInstm");
        ...
        return new ProductStreamItemByKeyItemReaderBuilder<StockInstrument>()
                .name("secXmlFileReader")
    ...
    private final Function<StockInstrument, ProductKey> toProductKey = stockInstrument -> {
    ...
    private final BiConsumer<StockInstrument, ProductStreamItem> afterBuildItem = ((stockInstrument, streamItem) -> {
        Map<String, Object> params = new HashMap<>();
        params.put("fileName", stockInstrument.getResource().getFilename());
        streamItem.setParams(params);
    });
}
