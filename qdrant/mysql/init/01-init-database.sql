...
CREATE TABLE IF NOT EXISTS business_knowledge (
  ...
  PRIMARY KEY (id),
  INDEX idx_bk_business_term (business_term),
  INDEX idx_bk_data_set_id (data_set_id),
  INDEX idx_bk_agent_id (agent_id),
  INDEX idx_bk_is_recall (is_recall),
...
CREATE TABLE IF NOT EXISTS semantic_model (
  ...
  PRIMARY KEY (id),
  INDEX idx_sm_agent_id (agent_id),
  INDEX idx_sm_field_name (field_name),
  INDEX idx_sm_status (status),
  INDEX idx_sm_is_recall (is_recall),
  FOREIGN KEY (agent_id) REFERENCES agent(id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='语义模型表';
...
CREATE TABLE IF NOT EXISTS agent_knowledge (
  ...
  INDEX idx_ak_title (title),
  INDEX idx_ak_type (type),
  INDEX idx_ak_status (status),
  INDEX idx_ak_category (category),
  INDEX idx_ak_embedding_status (embedding_status),
...
CREATE TABLE IF NOT EXISTS datasource (
  ...
  update_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (id),
  INDEX idx_ds_name (name),
  INDEX idx_ds_type (type),
  INDEX idx_ds_status (status),
  INDEX idx_ds_creator_id (creator_id)
...
CREATE TABLE IF NOT EXISTS agent_datasource (
  ...
  INDEX idx_ads_agent_id (agent_id),
  INDEX idx_ads_datasource_id (datasource_id),
...
CREATE TABLE IF NOT EXISTS agent_preset_question (
  ...
  update_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (id),
  INDEX idx_apq_agent_id (agent_id),
  INDEX idx_apq_sort_order (sort_order),
  INDEX idx_apq_is_active (is_active),
  FOREIGN KEY (agent_id) REFERENCES agent(id) ON DELETE CASCADE
