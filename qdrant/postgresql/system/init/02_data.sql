...
(1001, '基金产品类型',
 ...
 true, 'finance_fund', 2, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
...
(2001, 2, 'prodId', '产品ID,基金ID,Product ID,Fund ID', 'B_UT_PROD.PROD_ID',
 '基金产品唯一标识', 'B_UT_PROD 表的主键之一', 'integer', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2002, 2, 'prodName', '产品名称,基金名称,Product Name,Fund Name', 'B_UT_PROD.PROD_NAME',
 '基金产品中文名称', 'B_UT_PROD.PROD_NAME', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2003, 2, 'productTypeCode', '产品类型,基金类型,Type,Fund Type,UT', 'B_UT_PROD.PROD_TYPE_CDE',
 'PROD_TYPE_CDE=UT(FUND)', 'B_UT_PROD.PROD_TYPE_CDE', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2004, 2, 'productSubTypeCode', '产品子类型,Subtype,Sub Category', 'B_UT_PROD.PROD_SUBTP_CDE',
 '基金产品子类型代码', 'B_UT_PROD.PROD_SUBTP_CDE', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2005, 2, 'tradeCurrencyCode', '交易币种,交易货币,Trade CCY', 'B_UT_PROD.CCY_PROD_TRADE_CDE',
 '交易相关币种代码（CHAR(3)）', 'B_UT_PROD.CCY_PROD_TRADE_CDE', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2006, 2, 'productCurrencyCode', '产品币种,货币,CCY', 'B_UT_PROD.CCY_PROD_CDE',
 '产品币种（CHAR(3)）', 'B_UT_PROD.CCY_PROD_CDE', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2007, 2, 'riskLevelCode', '风险等级,风险级别,Risk Level,RL', 'B_UT_PROD.RISK_LVL_CDE',
 '基金产品风险等级代码（常见: L/M/H）', 'B_UT_PROD.RISK_LVL_CDE', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2008, 2, 'fundRetainMinAmount', '最低保留金额,最小持有,Retain Min', 'B_UT_PROD.FUND_RTAIN_MIN_AMT',
 '账户最低保留金额（离账后需保留的最小金额）', 'B_UT_PROD.FUND_RTAIN_MIN_AMT', 'decimal', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2009, 2, 'fundSwitchInMinAmount', '最小转换入金额,Switch-in Min', 'B_UT_PROD.FUND_SW_IN_MIN_AMT',
 '最小转换入金额', 'B_UT_PROD.FUND_SW_IN_MIN_AMT', 'decimal', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2010, 2, 'allowBuyIndicator', '可购买,允许购买,申购可用,Buyable', 'B_UT_PROD.ALLOW_BUY_PROD_IND',
 '是否允许申购/买入（指示位）', 'B_UT_PROD.ALLOW_BUY_PROD_IND', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2011, 2, 'allowSellIndicator', '可赎回,允许卖出,Sellable', 'B_UT_PROD.ALLOW_SELL_PROD_IND',
 '是否允许赎回/卖出（指示位）', 'B_UT_PROD.ALLOW_SELL_PROD_IND', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2012, 2, 'allowSwitchInIndicator', '可转换入,允许转换入,Switch In', 'B_UT_PROD.ALLOW_SW_IN_PROD_IND',
 '是否允许转换入（指示位）', 'B_UT_PROD.ALLOW_SW_IN_PROD_IND', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2013, 2, 'allowSwitchOutIndicator', '可转换出,允许转换出,Switch Out', 'B_UT_PROD.ALLOW_SW_OUT_PROD_IND',
 '是否允许转换出（指示位）', 'B_UT_PROD.ALLOW_SW_OUT_PROD_IND', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2014, 2, 'investIncrementMinAmount', '最小加码增量,加码增量,Increment Min', 'B_UT_PROD.INVST_INCRM_MIN_AMT',
 '追加投资的最小增量金额', 'B_UT_PROD.INVST_INCRM_MIN_AMT', 'decimal', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2015, 2, 'investMipIncrementMinAmount', '最小加码增量(MIP),MIP增量', 'B_UT_PROD.INVST_MIP_INCRM_MIN_AMT',
 'MIP（月供/定投）追加投资最小增量金额', 'B_UT_PROD.INVST_MIP_INCRM_MIN_AMT', 'decimal', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),
...
(2016, 2, 'channelCommonCode', '渠道通用代码,公共渠道码,Common Channel', 'B_UT_PROD_OFER_CHANL.CHANL_COMN_CDE',
 '产品可用的通用渠道代码（如网银/柜台/手机等统一编码）', 'B_UT_PROD_OFER_CHANL.CHANL_COMN_CDE', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2017, 2, 'channelCode', '渠道代码,销售渠道,办理渠道,Channel Code', 'B_UT_PROD_CHANL_ATTR.CHANL_CDE',
 '产品在某渠道的具体编码', 'B_UT_PROD_CHANL_ATTR.CHANL_CDE', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

-- 批处理（ACTV_DATA）
(2018, 2, 'jobBatchExecId', '批次ID,作业批次,Job Batch ID', 'ACTV_DATA.JOB_BATCH_EXEC_ID',
 '批处理作业执行批次ID', 'ACTV_DATA.JOB_BATCH_EXEC_ID', 'integer', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2019, 2, 'jobBatchTypeCode', '批次类型,作业类型,Batch Type', 'ACTV_DATA.JOB_BATCH_TYPE_CDE',
 '批处理作业类型代码（如 B_UT_WPC 等）', 'ACTV_DATA.JOB_BATCH_TYPE_CDE', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2020, 2, 'recordUpdateTime', '记录更新时间,更新时间,Record Update Time', 'ACTV_DATA.REC_UPDT_DT_TM',
 '记录的最后更新时间', 'ACTV_DATA.REC_UPDT_DT_TM', 'datetime', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

-- 客户（CUSTOMER）
(2021, 2, 'customerId', '客户ID,客户编号,Customer ID', 'CUSTOMER.CUSTOMER_ID',
 '客户标识（可能为外部或内部统一ID）', 'CUSTOMER.CUSTOMER_ID', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2022, 2, 'groupMemberCode', '集团成员,集团代码,Group Member', 'CUSTOMER.GROUP_MEMBER',
 '客户所属集团成员标识', 'CUSTOMER.GROUP_MEMBER', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2023, 2, 'countryCode', '国家代码,地区代码,Country Code', 'CUSTOMER.COUNTRY_CODE',
 '客户所在国家/地区代码', 'CUSTOMER.COUNTRY_CODE', 'string', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true),

(2024, 2, 'customerActive', '是否激活,有效标识,Active', 'CUSTOMER.ACTIVE',
 '客户激活状态（0/1 或 Y/N）', 'CUSTOMER.ACTIVE', 'integer', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, true, true);
