...
# 固定输出目录为 fundSearchResult
FUND_SEARCH_OUTPUT_DIR = BASE_DIR / "fundSearchResult"
...
async def capture_fund_screener_api(headless: bool = True, save_to_dir: Path = None) -> Dict[str, Any]:
    ...
    def should_capture_fund_search_request(request: Request) -> bool:
        ...
        # 只捕获特定的 fundSearchResult API
        target_api = 'investments3.personal-banking.hsbc.com.hk/shp/wealth-mobile-mds-shp-api-hk-hbap-prod-proxy/v0/wmds/fundSearchResult'

        return target_api in url
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 等待显示50条按钮 - 使用已验证有效的选择器，减少超时时间
            try:
                await page.wait_for_selector('a:has-text("50")', timeout=8000)
                print("✅ 找到显示50条按钮")
                log_debug("步骤4", "找到显示50条按钮", {
                    "selector": 'a:has-text("50")',
                    "element_type": "A标签",
                    "success": True
                })
            except Exception as e:
                print(f"❌ 未找到显示50条按钮: {e}")
                log_debug("步骤4", "显示50条按钮失败", {
                    "selector": 'a:has-text("50")',
                    "error": str(e)
                })
                return {
            ...
            # 在点击 "50" 按钮之前启用网络捕获
            print("🔄 启用 fundSearchResult API 捕获...")
            enable_capture()

            # 点击 "50" 按钮 - 使用已验证有效的选择器
            try:
                await page.click('a:has-text("50")')
                print("✅ 已点击显示50条按钮")
                log_debug("步骤4", "成功点击显示50条按钮", {
                    "selector": 'a:has-text("50")',
                    "method": "direct_click",
                    "success": True
                })
            except Exception as e:
                print(f"❌ 点击显示50条按钮失败: {e}")
                log_debug("步骤4", "点击显示50条按钮失败", {
                    "selector": 'a:has-text("50")',
                    "error": str(e)
                })
                disable_capture()
            ...
            # 拍摄最终状态截图
            if save_to_dir:
                screenshot_file = FUND_SEARCH_OUTPUT_DIR / f"fund_screener_result_{timestamp}.png"
...
async def main_fund_screener(headless: bool = True):
    """主函数：捕获基金筛选页面的 fundSearchResult API"""

    print("="*60)
    print(f"🚀 基金筛选器 API 捕获模式")
    ...
    if debug_logs:
        ...
        with open(debug_summary_file, 'w', encoding='utf-8') as f:
            ...
            f.write("4. 显示50条按钮: a:has-text(\"50\") (A标签)\n\n")
    ...
    # 保存调试日志
    if debug_logs:
        debug_log_file = screener_dir / "debug_logs.txt"
    ...
    files_created.append(f"fund_screener_result_{timestamp}.png")
