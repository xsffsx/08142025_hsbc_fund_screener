...
def cleanup_previous_results():
    ...
    # 保留 log 目录
    log_files = list(LOG_DIR.glob('*')) if LOG_DIR.exists() else []
    print(f"📋 保留 log 目录中的 {len(log_files)} 个日志文件")

    print(f"🎯 清理完成！共清理 {cleaned_files} 个文件，保留 {len(log_files)} 个日志文件\n")
