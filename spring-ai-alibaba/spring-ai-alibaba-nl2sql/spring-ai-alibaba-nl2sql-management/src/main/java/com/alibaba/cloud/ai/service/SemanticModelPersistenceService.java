...
public class SemanticModelPersistenceService {
	...
	public List<SemanticModel> getFieldByAgentId(Long agentId) {
		return this.jdbcTemplate.query(FIELD_GET_BY_AGENT_ID, new Object[] { String.valueOf(agentId) },
				(rs, rowNum) -> {
					SemanticModel model = new SemanticModel();
					model.setId(rs.getObject("id", Long.class));
					// agent_id在数据库中是varchar类型，需要转换为Long
					String agentIdStr = rs.getString("agent_id");
					model.setAgentId(agentIdStr != null ? Long.valueOf(agentIdStr) : null);
