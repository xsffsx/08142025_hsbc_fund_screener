...
public class AgentVectorService {
	...
	private com.alibaba.cloud.ai.connector.accessor.Accessor mysqlAccessor;

	@Autowired
	@Qualifier("postgreAccessor")
	private com.alibaba.cloud.ai.connector.accessor.Accessor postgreAccessor;

	@Autowired
	@Qualifier("oracleAccessor")
	private com.alibaba.cloud.ai.connector.accessor.Accessor oracleAccessor;
	...
	public List<String> getDatasourceTables(Integer datasourceId) {
		try {
			...
			// 检查数据源类型，支持MySQL、PostgreSQL和Oracle
			com.alibaba.cloud.ai.connector.accessor.Accessor dbAccessor;
			if ("mysql".equalsIgnoreCase(datasource.getType())) {
				dbAccessor = mysqlAccessor;
			}
			else if ("postgresql".equalsIgnoreCase(datasource.getType())
					|| "postgres".equalsIgnoreCase(datasource.getType())) {
				dbAccessor = postgreAccessor;
			}
			else if ("oracle".equalsIgnoreCase(datasource.getType())) {
				dbAccessor = oracleAccessor;
			}
			else {
				log.warn("Unsupported datasource type: {}, only MySQL, PostgreSQL and Oracle are supported currently",
						datasource.getType());
			...
			// 对于Oracle，schema是用户名；对于其他数据库，schema是数据库名
			if ("oracle".equalsIgnoreCase(datasource.getType())) {
				queryParam.setSchema(datasource.getUsername().toUpperCase());
			}
			else {
	...
	private DbConfig createDbConfigFromDatasource(com.alibaba.cloud.ai.entity.Datasource datasource) {
		...
		if ("mysql".equalsIgnoreCase(datasource.getType())) {
			...
			// Oracle的schema是用户名，通常大写
			dbConfig.setSchema(dbConfig.getUsername().toUpperCase());
		}
		// 其他数据库类型的支持可以在这里扩展
		// }

		// 设置Schema：Oracle已在上面设置为用户名，其他数据库使用数据库名称
		if (!"oracle".equalsIgnoreCase(datasource.getType())) {
