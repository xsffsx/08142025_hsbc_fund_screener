...
# Output paths relative to script location (05_performance_01_Calendar_returns directory)
SCRIPT_DIR = Path(__file__).resolve().parent
BASE_DIR = SCRIPT_DIR.parents[1]  # 项目根目录
DATA_DIR = BASE_DIR / f"data_{timestamp}"
DATA_DIR.mkdir(parents=True, exist_ok=True)

# 输出目录为脚本所在目录 (05_performance_01_Calendar_returns)
FUND_SEARCH_OUTPUT_DIR = SCRIPT_DIR
FUND_SEARCH_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# 子目录结构 - 在05_performance_01_Calendar_returns下创建
...
async def capture_fund_screener_api(headless: bool = True, save_to_dir: Path = None) -> Dict[str, Any]:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print(f"\n=== 步骤 4: 点击 Performance 标签 ===")
            # 等待页面加载完成，确保标签页可见
            await page.wait_for_timeout(2000)

            try:
                # 查找并点击 "Performance" 标签
                performance_selector = 'button:has-text("Performance"), a:has-text("Performance"), [data-testid*="performance"], [data-testid*="Performance"]'

                # 等待 Performance 标签出现
                await page.wait_for_selector(performance_selector, timeout=10000)
                print("✅ 找到 Performance 标签")

                # 点击 Performance 标签
                await page.click(performance_selector)
                print("✅ 已点击 Performance 标签")
                log_debug("步骤4", "成功点击Performance标签", {
                ...
                # 验证是否成功切换到 Performance 页面
                current_url = page.url
                print(f"✅ Performance 页面URL: {current_url}")

                # 确认点击成功 - 检查页面是否包含 Performance 相关内容
                performance_content = await page.query_selector('[data-testid*="performance"], .performance, #performance')
                if performance_content or "performance" in current_url.lower():
                    print("✅ 确认成功切换到 Performance 页面")
                    log_debug("步骤4", "Performance页面切换成功", {
                        "url": current_url,
                        "content_found": performance_content is not None,
                        "success": True
                    })
                else:
                    print("⚠️  无法确认是否成功切换到 Performance 页面，继续执行...")
                    log_debug("步骤4", "Performance页面切换状态不明", {
                        "url": current_url,
                        "content_found": False,
                        "warning": True
                    })
                ...
                print(f"\n=== 步骤 4.1: 点击 Calendar returns 子标签 ===")
                try:
                    # 查找并点击 "Calendar returns" 子标签
                    calendar_returns_selector = 'li:has-text("Calendar returns"), button:has-text("Calendar returns"), a:has-text("Calendar returns"), [data-testid*="calendar-returns"]'

                    # 等待 Calendar returns 子标签出现
                    await page.wait_for_selector(calendar_returns_selector, timeout=10000)
                    print("✅ 找到 Calendar returns 子标签")

                    # 点击 Calendar returns 子标签
                    await page.click(calendar_returns_selector)
                    print("✅ 已点击 Calendar returns 子标签")
                    log_debug("步骤4.1", "成功点击Calendar returns子标签", {
                        "selector": calendar_returns_selector,
                        "success": True
                    })

                    # 等待子标签页切换完成
                    await page.wait_for_timeout(3000)
                    print("✅ Calendar returns 子标签页加载完成")

                except Exception as e:
                    print(f"⚠️  点击 Calendar returns 子标签失败: {e}")
                    log_debug("步骤4.1", "Calendar returns子标签点击失败", {
                        "error": str(e),
                        "warning": True
                    })
                    print("⚠️  继续执行，可能在默认子标签页")

                print(f"\n=== 步骤 4.2: 点击 Fund 列标题进行排序 ===")
                try:
                    # 查找并点击 "Fund" 列标题
                    fund_selector = 'th:has-text("Fund"), button:has-text("Fund"), [data-testid*="fund"], .fund-header'
                ...
                print(f"❌ 点击 Performance 标签失败: {e}")
                log_debug("步骤4", "Performance标签点击失败", {
                    "error": str(e),
                    "selector": performance_selector
                })
                print("⚠️  继续执行，但可能无法获取 Performance 数据")
...
async def main_fund_screener(headless: bool = True):
    """主函数：捕获基金 Performance -> Calendar returns 页面的 API"""

    print("="*60)
    print(f"🚀 基金 Performance -> Calendar returns API 捕获模式")
    ...
    print(f"\n🎯 基金 Performance -> Calendar returns API 捕获完成！")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'screener':
        ...
        # 运行基金 Performance -> Calendar returns 捕获
