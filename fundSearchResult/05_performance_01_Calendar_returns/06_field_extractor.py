...
class FieldExtractor:
    ...
    def _get_nested_value(self, data: Dict[str, Any], path: str) -> Any:
        ...
        # 处理特殊路径：performance.calendarReturns.year[yearName=X].yearValue
        if "calendarReturns.year[yearName=" in path:
            # 提取yearName值
            import re
            match = re.search(r'year\[yearName=(\d+)\]\.yearValue', path)
            if match:
                year_name = int(match.group(1))
                performance_data = data.get("performance", {})
                calendar_returns = performance_data.get("calendarReturns", {})
                year_array = calendar_returns.get("year", [])

                for year_item in year_array:
                    if year_item.get("yearName") == year_name:
                        return year_item.get("yearValue")
                return None
