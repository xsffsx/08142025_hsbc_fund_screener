...
def map_fields_to_api(frontend_fields: Dict[str, str], sample_product: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    # YTD (年初至今收益率)
    return_ytd = sample_product.get("performance", {}).get("calendarReturns", {}).get("returnYTD", None)
    field_mapping["YTD"] = {
        "frontend_name": "YTD",
        "frontend_description": "年初至今收益率",
        "api_path": "performance.calendarReturns.returnYTD",
        "api_value": return_ytd,
        "data_type": "percentage",
        "example": f"{return_ytd:.2f}%" if return_ytd is not None else "N/A"
    }

    # 获取年度收益率数据 (从year数组中提取)
    calendar_years = sample_product.get("performance", {}).get("calendarReturns", {}).get("year", [])

    # 2024 (最近1年收益率 - yearName: 1)
    return_2024 = None
    for year_data in calendar_years:
        if year_data.get("yearName") == 1:
            return_2024 = year_data.get("yearValue")
            break
    field_mapping["2024"] = {
        "frontend_name": "2024",
        "frontend_description": "2024年收益率",
        "api_path": "performance.calendarReturns.year[yearName=1].yearValue",
        "api_value": return_2024,
        "data_type": "percentage",
        "example": f"{return_2024:.2f}%" if return_2024 is not None else "N/A"
    }

    # 2023 (最近2年收益率 - yearName: 2)
    return_2023 = None
    for year_data in calendar_years:
        if year_data.get("yearName") == 2:
            return_2023 = year_data.get("yearValue")
            break
    field_mapping["2023"] = {
        "frontend_name": "2023",
        "frontend_description": "2023年收益率",
        "api_path": "performance.calendarReturns.year[yearName=2].yearValue",
        "api_value": return_2023,
        "data_type": "percentage",
        "example": f"{return_2023:.2f}%" if return_2023 is not None else "N/A"
    }

    # 2022 (最近3年收益率 - yearName: 3)
    return_2022 = None
    for year_data in calendar_years:
        if year_data.get("yearName") == 3:
            return_2022 = year_data.get("yearValue")
            break
    field_mapping["2022"] = {
        "frontend_name": "2022",
        "frontend_description": "2022年收益率",
        "api_path": "performance.calendarReturns.year[yearName=3].yearValue",
        "api_value": return_2022,
        "data_type": "percentage",
        "example": f"{return_2022:.2f}%" if return_2022 is not None else "N/A"
    }

    # 2021 (最近4年收益率 - yearName: 4)
    return_2021 = None
    for year_data in calendar_years:
        if year_data.get("yearName") == 4:
            return_2021 = year_data.get("yearValue")
            break
    field_mapping["2021"] = {
        "frontend_name": "2021",
        "frontend_description": "2021年收益率",
        "api_path": "performance.calendarReturns.year[yearName=4].yearValue",
        "api_value": return_2021,
        "data_type": "percentage",
        "example": f"{return_2021:.2f}%" if return_2021 is not None else "N/A"
    }

    # 2020 (最近5年收益率 - yearName: 5)
    return_2020 = None
    for year_data in calendar_years:
        if year_data.get("yearName") == 5:
            return_2020 = year_data.get("yearValue")
            break
    field_mapping["2020"] = {
        "frontend_name": "2020",
        "frontend_description": "2020年收益率",
        "api_path": "performance.calendarReturns.year[yearName=5].yearValue",
        "api_value": return_2020,
        "data_type": "percentage",
        "example": f"{return_2020:.2f}%" if return_2020 is not None else "N/A"
    }
...
**请求方法**: POST"""
    ...
    report = f"""# HSBC Fund Screener - Performance -> Calendar returns 字段映射分析报告
...
"""
    ...
    for i, (field_key, mapping) in enumerate(field_mapping.items(), 1):
        ...
        field_descriptions = {
            ...
            'YTD': '年初至今收益率，基金当年表现指标',
            '2024': '2024年收益率，基金2024年度表现',
            '2023': '2023年收益率，基金2023年度表现',
            '2022': '2022年收益率，基金2022年度表现',
            '2021': '2021年收益率，基金2021年度表现',
            '2020': '2020年收益率，基金2020年度表现'
        }

        field_remarks = {
            'Fund': '基金名称是用户识别产品的主要标识，通常包含基金公司、投资策略和份额类别信息',
            'YTD': '年初至今收益率反映基金在当前年度的表现，是评估基金短期表现的重要指标',
            '2024': '2024年收益率反映基金在2024年度的完整表现，用于评估基金的年度投资效果',
            '2023': '2023年收益率反映基金在2023年度的完整表现，用于历史年度对比分析',
            '2022': '2022年收益率反映基金在2022年度的完整表现，用于历史年度对比分析',
            '2021': '2021年收益率反映基金在2021年度的完整表现，用于历史年度对比分析',
            '2020': '2020年收益率反映基金在2020年度的完整表现，包含疫情期间的市场表现'
        }
