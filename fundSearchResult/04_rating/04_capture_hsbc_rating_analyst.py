...
async def capture_fund_screener_api(headless: bool = True, save_to_dir: Path = None) -> Dict[str, Any]:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            try:
                # 查找并点击 "Rating" 标签
                rating_selector = 'button:has-text("Rating"), a:has-text("Rating"), [data-testid*="rating"], [data-testid*="Rating"]'

                # 等待 Rating 标签出现
                await page.wait_for_selector(rating_selector, timeout=10000)
                print("✅ 找到 Rating 标签")

                # 点击 Rating 标签
                await page.click(rating_selector)
                print("✅ 已点击 Rating 标签")
                log_debug("步骤4", "成功点击Rating标签", {
                ...
                print(f"✅ Rating 页面URL: {current_url}")

                # 确认点击成功 - 检查页面是否包含 Rating 相关内容
                rating_content = await page.query_selector('[data-testid*="rating"], .rating, #rating')
                if rating_content or "rating" in current_url.lower():
                    print("✅ 确认成功切换到 Rating 页面")
                    log_debug("步骤4", "Rating页面切换成功", {
                        "url": current_url,
                        "content_found": rating_content is not None,
                        "success": True
                    })
                else:
                    print("⚠️  无法确认是否成功切换到 Rating 页面，继续执行...")
                    log_debug("步骤4", "Rating页面切换状态不明", {
                        "url": current_url,
                        "content_found": False,
                        "warning": True
                    })
                ...
                print(f"\n=== 步骤 4.1: 点击 Average credit quality 列标题 ===")
                try:
                    # 查找并点击 "Average credit quality" 列标题
                    credit_quality_selector = 'th:has-text("Average credit quality"), button:has-text("Average credit quality"), [data-testid*="credit"], .credit-quality'

                    # 等待 Average credit quality 列标题出现
                    await page.wait_for_selector(credit_quality_selector, timeout=10000)
                    print("✅ 找到 Average credit quality 列标题")

                    # 点击 Average credit quality 列标题进行排序
                    await page.click(credit_quality_selector)
                    print("✅ 已点击 Average credit quality 列标题")
                    log_debug("步骤4.1", "成功点击Average credit quality列标题", {
                        "selector": credit_quality_selector,
                        "success": True
                    })
                    ...
                    print(f"⚠️  点击 Average credit quality 列标题失败: {e}")
                    log_debug("步骤4.1", "Average credit quality列标题点击失败", {
                        "error": str(e),
                        "warning": True
                    })
                    print("⚠️  继续执行，使用原始数据")
                ...
                print(f"❌ 点击 Rating 标签失败: {e}")
                log_debug("步骤4", "Rating标签点击失败", {
                    "error": str(e),
                    "selector": rating_selector
                })
                print("⚠️  继续执行，但可能无法获取 Rating 数据")
...
async def main_fund_screener(headless: bool = True):
    """主函数：捕获基金 Rating 页面的 API"""

    print("="*60)
    print(f"🚀 基金 Rating API 捕获模式")
