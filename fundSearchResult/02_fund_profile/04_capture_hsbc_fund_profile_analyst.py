...
async def capture_fund_screener_api(headless: bool = True, save_to_dir: Path = None) -> Dict[str, Any]:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print(f"\n=== 步骤 4: 点击 Fund profile 标签 ===")
            # 等待页面加载完成，确保标签页可见
            await page.wait_for_timeout(2000)

            try:
                # 查找并点击 "Fund profile" 标签
                fund_profile_selector = 'button:has-text("Fund profile"), a:has-text("Fund profile"), [data-testid*="fund-profile"], [data-testid*="Fund-profile"]'

                # 等待 Fund profile 标签出现
                await page.wait_for_selector(fund_profile_selector, timeout=10000)
                print("✅ 找到 Fund profile 标签")

                # 点击 Fund profile 标签
                await page.click(fund_profile_selector)
                print("✅ 已点击 Fund profile 标签")
                log_debug("步骤4", "成功点击Fund profile标签", {
                    "selector": fund_profile_selector,
                    "success": True
                })
                ...
                # 验证是否成功切换到 Fund profile 页面
                current_url = page.url
                print(f"✅ Fund profile 页面URL: {current_url}")

                # 确认点击成功 - 检查页面是否包含 Fund profile 相关内容
                fund_profile_content = await page.query_selector('[data-testid*="fund-profile"], .fund-profile, #fund-profile')
                if fund_profile_content or "profile" in current_url.lower():
                    print("✅ 确认成功切换到 Fund profile 页面")
                    log_debug("步骤4", "Fund profile页面切换成功", {
                        "url": current_url,
                        "content_found": fund_profile_content is not None,
                        "success": True
                    })
                else:
                    print("⚠️  无法确认是否成功切换到 Fund profile 页面，继续执行...")
                    log_debug("步骤4", "Fund profile页面切换状态不明", {
                        "url": current_url,
                        "content_found": False,
                        "warning": True
                    })
                ...
                print(f"❌ 点击 Fund profile 标签失败: {e}")
                log_debug("步骤4", "Fund profile标签点击失败", {
                    "error": str(e),
                    "selector": fund_profile_selector
                })
                print("⚠️  继续执行，但可能无法获取 Fund profile 数据")
...
async def main_fund_screener(headless: bool = True):
    """主函数：捕获基金 Fund profile 页面的 API"""

    print("="*60)
    print(f"🚀 基金 Fund Profile API 捕获模式")
