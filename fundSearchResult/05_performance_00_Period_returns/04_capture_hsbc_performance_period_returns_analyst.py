    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                print(f"\n=== 步骤 4.1: 点击 10 year 列标题 ===")
                try:
                    # 查找并点击 "10 year" 列标题
                    ten_year_selector = 'th:has-text("10 year"), button:has-text("10 year"), [data-testid*="10-year"], .ten-year'

                    # 等待 10 year 列标题出现
                    await page.wait_for_selector(ten_year_selector, timeout=10000)
                    print("✅ 找到 10 year 列标题")

                    # 点击 10 year 列标题进行排序
                    await page.click(ten_year_selector)
                    print("✅ 已点击 10 year 列标题")
                    log_debug("步骤4.1", "成功点击10 year列标题", {
                    ...
                    print(f"⚠️  点击 10 year 列标题失败: {e}")
                    log_debug("步骤4.1", "10 year列标题点击失败", {
                        "error": str(e),
                        "warning": True
                    })
                    print("⚠️  继续执行，使用原始数据")
...
async def main_fund_screener(headless: bool = True):
    """主函数：捕获基金 Performance -> Period returns 页面的 API"""

    print("="*60)
    print(f"🚀 基金 Performance -> Period returns API 捕获模式")
    ...
    print(f"\n🎯 基金 Performance -> Period returns API 捕获完成！")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'screener':
        ...
        # 运行基金 Performance -> Period returns 捕获
