"""
HSBC基金筛选器 - Holdings -> Asset allocation 子标签页数据捕获脚本

功能：
1. 访问HSBC基金筛选页面，设置风险等级为"Speculative - 5"
2. 点击"Holdings"标签页
3. 保持"Asset allocation"子标签（默认显示，无需点击）
4. 捕获页面数据和截图
5. 保存API响应数据到JSON文件
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                try:
                    # 查找并点击 "Stock" 列标题
                    stock_selector = 'th:has-text("Stock"), button:has-text("Stock"), [data-testid*="stock"], .stock'

                    # 等待 Stock 列标题出现
                    await page.wait_for_selector(stock_selector, timeout=10000)
                    print("✅ 找到 Stock 列标题")

                    # 点击 Stock 列标题进行排序
                    await page.click(stock_selector)
                    print("✅ 已点击 Stock 列标题")
                    log_debug("步骤4.1", "成功点击Stock列标题", {
                        "selector": stock_selector,
                        "success": True
                    })
                    ...
                    print(f"⚠️  点击 Stock 列标题失败: {e}")
                    log_debug("步骤4.1", "Stock列标题点击失败", {
                        "error": str(e),
                        "warning": True
                    })
                    print("⚠️  继续执行，使用原始数据")
...
async def main_fund_screener(headless: bool = True):
    """主函数：捕获基金 Holdings -> Asset allocation 页面的 API"""

    print("="*60)
    print(f"🚀 基金 Holdings -> Asset allocation API 捕获模式")
    ...
    print(f"\n🎯 基金 Holdings -> Asset allocation API 捕获完成！")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'screener':
        ...
        # 运行基金 Holdings -> Asset allocation 捕获
