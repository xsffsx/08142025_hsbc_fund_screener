...
def map_fields_to_api(frontend_fields: Dict[str, str], sample_product: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    # 获取资产配置数据
    asset_allocations = sample_product.get("holdings", {}).get("assetAlloc", {}).get("assetAllocations", [])

    # 创建资产配置映射字典
    asset_map = {}
    for allocation in asset_allocations:
        name = allocation.get("name", "")
        weighting = allocation.get("weighting", None)
        asset_map[name] = weighting

    # Stock (股票配置)
    stock_weighting = asset_map.get("Stock", None)
    field_mapping["Stock"] = {
        "frontend_name": "Stock",
        "frontend_description": "股票资产配置比例",
        "api_path": "holdings.assetAlloc.assetAllocations[name=Stock].weighting",
        "api_value": stock_weighting,
        "data_type": "percentage",
        "example": f"{stock_weighting:.2f}%" if stock_weighting is not None else "N/A"
    }

    # Bond (债券配置)
    bond_weighting = asset_map.get("Bond", None)
    field_mapping["Bond"] = {
        "frontend_name": "Bond",
        "frontend_description": "债券资产配置比例",
        "api_path": "holdings.assetAlloc.assetAllocations[name=Bond].weighting",
        "api_value": bond_weighting,
        "data_type": "percentage",
        "example": f"{bond_weighting:.2f}%" if bond_weighting is not None else "N/A"
    }

    # Cash (现金配置)
    cash_weighting = asset_map.get("Cash", None)
    field_mapping["Cash"] = {
        "frontend_name": "Cash",
        "frontend_description": "现金资产配置比例",
        "api_path": "holdings.assetAlloc.assetAllocations[name=Cash].weighting",
        "api_value": cash_weighting,
        "data_type": "percentage",
        "example": f"{cash_weighting:.2f}%" if cash_weighting is not None else "N/A"
    }

    # Others (其他配置) - 通过计算得出
    stock_weighting = asset_map.get("Stock", 0)
    bond_weighting = asset_map.get("Bond", 0)
    cash_weighting = asset_map.get("Cash", 0)
    calculated_others = 100 - stock_weighting - bond_weighting - cash_weighting

    field_mapping["Others"] = {
        "frontend_name": "Others",
        "frontend_description": "其他资产配置比例 (通过计算得出: 100% - Stock% - Bond% - Cash%)",
        "api_path": "calculated:100-Stock-Bond-Cash",
        "api_value": calculated_others,
        "data_type": "percentage",
        "example": f"{calculated_others:.2f}%" if calculated_others is not None else "N/A",
        "is_calculated": True,
        "calculation_formula": "100% - Stock% - Bond% - Cash%",
        "calculation_dependencies": ["Stock", "Bond", "Cash"]
    }
...
"""
    ...
    for i, (field_key, mapping) in enumerate(field_mapping.items(), 1):
        ...
        field_descriptions = {
            ...
            'Stock': '股票资产配置比例，反映基金投资于股票市场的资金占比',
            'Bond': '债券资产配置比例，反映基金投资于债券市场的资金占比',
            'Cash': '现金资产配置比例，反映基金持有现金及现金等价物的资金占比',
            'Others': '其他资产配置比例，反映基金投资于其他类型资产的资金占比'
        }

        field_remarks = {
            'Fund': '基金名称是用户识别产品的主要标识，通常包含基金公司、投资策略和份额类别信息',
            'Stock': '股票配置比例直接影响基金的风险收益特征，高股票配置通常意味着更高的潜在收益和风险',
            'Bond': '债券配置比例反映基金的稳健程度，债券通常提供相对稳定的收益和较低的波动性',
            'Cash': '现金配置比例体现基金的流动性管理策略，适度的现金配置有助于应对赎回需求和投资机会',
            'Others': '其他资产配置包括商品、房地产投资信托基金(REITs)、衍生品等，用于分散投资风险和增强收益'
        }
