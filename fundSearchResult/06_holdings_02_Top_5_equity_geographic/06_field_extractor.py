...
class FieldExtractor:
    ...
    def _load_field_mapping(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "1st": {
                "api_path": "calculated:top_geographic[0]",
                "data_type": "string"
            },
            "2nd": {
                "api_path": "calculated:top_geographic[1]",
                "data_type": "string"
            },
            "3rd": {
                "api_path": "calculated:top_geographic[2]",
                "data_type": "string"
            },
            "4th": {
                "api_path": "calculated:top_geographic[3]",
                "data_type": "string"
            },
            "5th": {
                "api_path": "calculated:top_geographic[4]",
                "data_type": "string"
            }
        }
    ...
    def _get_nested_value(self, data: Dict[str, Any], path: str) -> Any:
        """从嵌套字典中获取值"""
        # 处理top geographic排序字段：calculated:top_geographic[index]
        if path.startswith("calculated:top_geographic[") and path.endswith("]"):
            # 提取索引
            import re
            match = re.search(r'top_geographic\[(\d+)\]', path)
            if match:
                index = int(match.group(1))

                # 获取equityRegional数据并按权重排序
                holdings_data = data.get("holdings", {})
                equity_regional_data = holdings_data.get("equityRegional", {})
