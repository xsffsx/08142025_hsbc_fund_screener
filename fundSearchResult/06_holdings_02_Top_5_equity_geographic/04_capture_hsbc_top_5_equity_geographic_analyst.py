"""
HSBC基金筛选器 - Holdings -> Top 5 equity geographic 子标签页数据捕获脚本

功能：
1. 访问HSBC基金筛选页面，设置风险等级为"Speculative - 5"
2. 点击"Holdings"标签页
3. 点击"Top 5 equity geographic"子标签
4. 点击Fund列标题进行排序
5. 捕获页面数据和截图
6. 保存API响应数据到JSON文件
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                print(f"\n=== 步骤 4.1: 点击 Top 5 equity geographic 子标签 ===")
                try:
                    # 查找并点击 "Top 5 equity geographic" 子标签
                    # 根据您提供的HTML结构，使用li元素和文本匹配
                    geographic_selector = 'li:has-text("Top 5 equity geographic"), button:has-text("Top 5 equity geographic"), a:has-text("Top 5 equity geographic")'

                    # 等待 Top 5 equity geographic 子标签出现
                    await page.wait_for_selector(geographic_selector, timeout=10000)
                    print("✅ 找到 Top 5 equity geographic 子标签")

                    # 点击 Top 5 equity geographic 子标签
                    await page.click(geographic_selector)
                    print("✅ 已点击 Top 5 equity geographic 子标签")
                    log_debug("步骤4.1", "成功点击Top 5 equity geographic子标签", {
                        "selector": geographic_selector,
                        "action": "click_subtab",
                        "success": True
                    })
                    ...
                    # 等待子标签页面完全加载和新的API请求
                    await page.wait_for_timeout(3000)
                    print("✅ 等待Top 5 equity geographic页面加载完成和新API请求")

                except Exception as e:
                    print(f"⚠️  点击 Top 5 equity geographic 子标签失败: {e}")
                    log_debug("步骤4.1", "Top 5 equity geographic子标签点击失败", {
                        "error": str(e),
                        "warning": True
                    })
                    print("⚠️  继续执行，使用原始数据")
...
async def main_fund_screener(headless: bool = True):
    """主函数：捕获基金 Holdings -> Top 5 equity geographic 页面的 API"""

    print("="*60)
    print(f"🚀 基金 Holdings -> Top 5 equity geographic API 捕获模式")
    ...
    print(f"\n🎯 基金 Holdings -> Top 5 equity geographic API 捕获完成！")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'screener':
        ...
        # 运行基金 Holdings -> Top 5 equity geographic 捕获
