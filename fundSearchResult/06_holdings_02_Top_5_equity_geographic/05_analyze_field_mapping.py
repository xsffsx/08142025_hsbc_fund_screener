...
def map_fields_to_api(frontend_fields: Dict[str, str], sample_product: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    # 获取Top 5 equity geographic数据
    equity_regional_data = sample_product.get("holdings", {}).get("equityRegional", {})
    regional_exposures = equity_regional_data.get("regionalExposures", [])

    # 创建地理区域权重列表，过滤掉Others和权重为0的项
    regions = []
    geographic_mapping = {
        "CA": "Canada",
        "US": "United States",
        ...
        "JAPAN": "Japan",
        "CHINA": "China",
        "ASIA_DEV": "Asia - Developed",
        "ASIA_EM": "Asia - Emerging",
        "LATIN": "Latin America"
    }

    for region in regional_exposures:
        region_code = region.get("name", "")
        weight = region.get("weighting", 0)

        # 跳过Others和权重为0的项
        if region_code != "Others" and weight > 0:
            display_name = geographic_mapping.get(region_code, region_code)
            regions.append({"name": display_name, "weight": weight, "api_key": region_code})

    # 按权重排序，获取前5个地理区域
    sorted_regions = sorted(regions, key=lambda x: x["weight"], reverse=True)[:5]

    # 1st (第一大地理区域)
    if len(sorted_regions) > 0:
        first_region_name = sorted_regions[0]["name"]
        first_region_weight = sorted_regions[0]["weight"]
        first_api_path = f"holdings.equityRegional.regionalExposures[name={sorted_regions[0]['api_key']}]"
    else:
        first_region_name = ""
        first_region_weight = ""
        first_api_path = "N/A"
    ...
    field_mapping["1st"] = {
        "frontend_name": "1st",
        "frontend_description": "第一大股票地理区域配置，包含地理区域名称和权重百分比",
        "api_path": first_api_path,
        "api_value": first_region_name,
        "api_value_2": f"{first_region_weight:.2f}%" if first_region_weight else "",
        "data_type": "string",
        "example": first_region_name if first_region_name else "N/A",
        "example_2": f"{first_region_weight:.2f}%" if first_region_weight else "N/A"
    }

    # 2nd (第二大地理区域)
    if len(sorted_regions) > 1:
        second_region_name = sorted_regions[1]["name"]
        second_region_weight = sorted_regions[1]["weight"]
        second_api_path = f"holdings.equityRegional.regionalExposures[name={sorted_regions[1]['api_key']}]"
    else:
        second_region_name = ""
        second_region_weight = ""
        second_api_path = "N/A"

    field_mapping["2nd"] = {
        "frontend_name": "2nd",
        "frontend_description": "第二大股票地理区域配置，包含地理区域名称和权重百分比",
        "api_path": second_api_path,
        "api_value": second_region_name,
        "api_value_2": f"{second_region_weight:.2f}%" if second_region_weight else "",
        "data_type": "string",
        "example": second_region_name if second_region_name else "N/A",
        "example_2": f"{second_region_weight:.2f}%" if second_region_weight else "N/A"
    }

    # 3rd (第三大地理区域)
    if len(sorted_regions) > 2:
        third_region_name = sorted_regions[2]["name"]
        third_region_weight = sorted_regions[2]["weight"]
        third_api_path = f"holdings.equityRegional.regionalExposures[name={sorted_regions[2]['api_key']}]"
    else:
        third_region_name = ""
        third_region_weight = ""
        third_api_path = "N/A"
    ...
    field_mapping["3rd"] = {
        "frontend_name": "3rd",
        "frontend_description": "第三大股票地理区域配置，包含地理区域名称和权重百分比",
        "api_path": third_api_path,
        "api_value": third_region_name,
        "api_value_2": f"{third_region_weight:.2f}%" if third_region_weight else "",
        "data_type": "string",
        "example": third_region_name if third_region_name else "N/A",
        "example_2": f"{third_region_weight:.2f}%" if third_region_weight else "N/A"
    }

    # 4th (第四大地理区域)
    if len(sorted_regions) > 3:
        fourth_region_name = sorted_regions[3]["name"]
        fourth_region_weight = sorted_regions[3]["weight"]
        fourth_api_path = f"holdings.equityRegional.regionalExposures[name={sorted_regions[3]['api_key']}]"
    else:
        fourth_region_name = ""
        fourth_region_weight = ""
        fourth_api_path = "N/A"

    field_mapping["4th"] = {
        "frontend_name": "4th",
        "frontend_description": "第四大股票地理区域配置，包含地理区域名称和权重百分比",
        "api_path": fourth_api_path,
        "api_value": fourth_region_name,
        "api_value_2": f"{fourth_region_weight:.2f}%" if fourth_region_weight else "",
        "data_type": "string",
        "example": fourth_region_name if fourth_region_name else "N/A",
        "example_2": f"{fourth_region_weight:.2f}%" if fourth_region_weight else "N/A"
    }

    # 5th (第五大地理区域)
    if len(sorted_regions) > 4:
        fifth_region_name = sorted_regions[4]["name"]
        fifth_region_weight = sorted_regions[4]["weight"]
        fifth_api_path = f"holdings.equityRegional.regionalExposures[name={sorted_regions[4]['api_key']}]"
    else:
        fifth_region_name = ""
        fifth_region_weight = ""
        fifth_api_path = "N/A"
    ...
    field_mapping["5th"] = {
        "frontend_name": "5th",
        "frontend_description": "第五大股票地理区域配置，包含地理区域名称和权重百分比",
        "api_path": fifth_api_path,
        "api_value": fifth_region_name,
        "api_value_2": f"{fifth_region_weight:.2f}%" if fifth_region_weight else "",
        "data_type": "string",
        "example": fifth_region_name if fifth_region_name else "N/A",
        "example_2": f"{fifth_region_weight:.2f}%" if fifth_region_weight else "N/A"
    }
...
"""
    ...
    for i, (field_key, mapping) in enumerate(field_mapping.items(), 1):
        ...
        field_descriptions = {
            ...
            '1st': '第一大股票地理区域配置，基金在股票投资中权重最高的地理区域',
            '2nd': '第二大股票地理区域配置，基金在股票投资中权重第二的地理区域',
            '3rd': '第三大股票地理区域配置，基金在股票投资中权重第三的地理区域',
            '4th': '第四大股票地理区域配置，基金在股票投资中权重第四的地理区域',
            '5th': '第五大股票地理区域配置，基金在股票投资中权重第五的地理区域'
        }

        field_remarks = {
            'Fund': '基金名称是用户识别产品的主要标识，通常包含基金公司、投资策略和份额类别信息',
            '1st': '第一大地理区域配置反映基金的核心投资市场，通常占股票投资的最大比重，是基金地理投资策略的重要体现',
            '2nd': '第二大地理区域配置显示基金的次要投资市场，与第一大区域形成投资组合的主要构成，有助于分散单一市场风险',
            '3rd': '第三大地理区域配置进一步丰富基金的地理分布，提供额外的投资机会和地域风险分散效果',
            '4th': '第四大地理区域配置体现基金的全球化投资策略，通过更广泛的地理分布降低区域集中度风险',
            '5th': '第五大地理区域配置完善基金的地理配置结构，展示基金在不同经济体的投资布局和全球风险管理能力'
        }
        ...
        # 处理双值字段（地理区域名称 + 权重百分比）
        if 'api_value_2' in mapping and mapping['api_value_2']:
            api_example_2 = f"`{mapping['api_value_2']}`"
            frontend_example_2 = mapping.get('example_2', 'N/A')
        else:
            api_example_2 = ""
            frontend_example_2 = ""
        ...
        if 'api_value_2' in mapping and mapping['api_value_2']:
            # 构建API路径1和API路径2
            api_path_1 = f"{mapping['api_path']}.name"  # 地理区域名称路径
            api_path_2 = f"{mapping['api_path']}.weighting"  # 权重百分比路径
...
| **示例对比1** | {mapping['example']} | {api_example} | 地理区域名称 |
| **API路径1** | - | `{api_path_1}` | 地理区域名称的API路径 |
| **示例对比2** | {frontend_example_2} | {api_example_2} | 权重百分比 |
| **API路径2** | - | `{api_path_2}` | 权重百分比的API路径 |
| **备注** | {field_remark} | | |"""
        else:
            report += f"""### {i}. {mapping['frontend_name']} ({mapping['frontend_description']})
