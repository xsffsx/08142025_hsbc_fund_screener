"""
HSBC基金筛选器 - Holdings -> Top 5 bond geographic 子标签页数据捕获脚本

功能：
1. 访问HSBC基金筛选页面，设置风险等级为"Speculative - 5"
2. 点击"Holdings"标签页
3. 点击"Top 5 bond geographic"子标签
4. 点击Fund列标题进行排序
5. 捕获页面数据和截图
6. 保存API响应数据到JSON文件

输出文件：
- result/fund_screener_result_*.png: 页面截图
- result/fund_screener_page_*.html: 页面HTML源码
- result/fund_screener_info_*.json: 页面信息
- result/fundSearchResult_data.json: API响应数据
"""
...
# Output paths relative to script location (06_holdings_02_Top_5_equity_geographic directory)
SCRIPT_DIR = Path(__file__).resolve().parent
BASE_DIR = SCRIPT_DIR.parents[1]  # 项目根目录
DATA_DIR = BASE_DIR / f"data_{timestamp}"
DATA_DIR.mkdir(parents=True, exist_ok=True)

# 输出目录为脚本所在目录 (06_holdings_02_Top_5_equity_geographic)
FUND_SEARCH_OUTPUT_DIR = SCRIPT_DIR
FUND_SEARCH_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# 子目录结构 - 在06_holdings_02_Top_5_equity_geographic下创建
...
async def capture_fund_screener_api(headless: bool = True, save_to_dir: Path = None) -> Dict[str, Any]:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            try:
                # 查找并点击 "Holdings" 标签
                holdings_selector = 'button:has-text("Holdings"), a:has-text("Holdings"), [data-testid*="holdings"], [data-testid*="Holdings"]'

                # 等待 Holdings 标签出现
                await page.wait_for_selector(holdings_selector, timeout=10000)
                print("✅ 找到 Holdings 标签")

                # 点击 Holdings 标签
                await page.click(holdings_selector)
                print("✅ 已点击 Holdings 标签")
                log_debug("步骤4", "成功点击Holdings标签", {
                    "selector": holdings_selector,
                    "success": True
                })
                ...
                # 验证是否成功切换到 Holdings 页面
                current_url = page.url
                print(f"✅ Holdings 页面URL: {current_url}")

                # 确认点击成功 - 检查页面是否包含 Holdings 相关内容
                holdings_content = await page.query_selector('[data-testid*="holdings"], .holdings, #holdings')
                if holdings_content or "holdings" in current_url.lower():
                    print("✅ 确认成功切换到 Holdings 页面")
                    log_debug("步骤4", "Holdings页面切换成功", {
                        "url": current_url,
                        "content_found": holdings_content is not None,
                        "success": True
                    })
                else:
                    print("⚠️  无法确认是否成功切换到 Holdings 页面，继续执行...")
                    log_debug("步骤4", "Holdings页面切换状态不明", {
                        "url": current_url,
                        "content_found": False,
                        "warning": True
                    })
                ...
                print(f"\n=== 步骤 4.1: 点击 Top 5 bond geographic 子标签 ===")
                try:
                    # 查找并点击 "Top 5 bond geographic" 子标签
                    # 根据您提供的HTML结构，使用li元素和文本匹配
                    bond_geographic_selector = 'li:has-text("Top 5 bond geographic"), button:has-text("Top 5 bond geographic"), a:has-text("Top 5 bond geographic")'

                    # 等待 Top 5 bond geographic 子标签出现
                    await page.wait_for_selector(bond_geographic_selector, timeout=10000)
                    print("✅ 找到 Top 5 bond geographic 子标签")

                    # 点击 Top 5 bond geographic 子标签
                    await page.click(bond_geographic_selector)
                    print("✅ 已点击 Top 5 bond geographic 子标签")
                    log_debug("步骤4.1", "成功点击Top 5 bond geographic子标签", {
                        "selector": bond_geographic_selector,
                        "action": "click_subtab",
                        "success": True
                    })
                    ...
                    # 等待子标签页面完全加载和新的API请求
                    await page.wait_for_timeout(3000)
                    print("✅ 等待Top 5 bond geographic页面加载完成和新API请求")

                except Exception as e:
                    print(f"⚠️  点击 Top 5 bond geographic 子标签失败: {e}")
                    log_debug("步骤4.1", "Top 5 bond geographic子标签点击失败", {
                        "error": str(e),
                        "warning": True
                    })
                    print("⚠️  继续执行，使用原始数据")

                # 步骤 4.2: 点击Fund列标题进行排序
                print(f"\n=== 步骤 4.2: 点击Fund列标题进行排序 ===")
                try:
                    # 查找并点击Fund列标题
                    ...
                    # 等待Fund列标题出现
                    await page.wait_for_selector(fund_selector, timeout=10000)
                    print("✅ 找到Fund列标题")

                    # 点击Fund列标题进行排序
                    await page.click(fund_selector)
                    print("✅ 已点击Fund列标题进行排序")
                    log_debug("步骤4.2", "成功点击Fund列标题排序", {
                    ...
                    # 等待排序完成和新的API请求
                    await page.wait_for_timeout(3000)
                    print("✅ 等待Fund列排序完成和新API请求")

                except Exception as e:
                    print(f"⚠️  点击Fund列标题排序失败: {e}")
                    log_debug("步骤4.2", "Fund列标题排序失败", {
                        "error": str(e),
                        "warning": True
                    })
                    print("⚠️  继续执行，使用原始数据")

                print(f"\n=== 步骤 4.2: 跳过 Fund 列标题排序 ===")
                print("⏭️  跳过点击Fund列标题排序，直接使用默认排序")
                log_debug("步骤4.2", "跳过Fund列标题排序", {
                    "action": "skip_fund_sort",
                    "reason": "使用默认排序获取地理区域数据",
                    "success": True
                })
                ...
                print(f"❌ 点击 Holdings 标签失败: {e}")
                log_debug("步骤4", "Holdings标签点击失败", {
                    "error": str(e),
                    "selector": holdings_selector
                })
                print("⚠️  继续执行，但可能无法获取 Holdings 数据")
...
async def main_fund_screener(headless: bool = True):
    """主函数：捕获基金 Holdings -> Top 5 bond geographic 页面的 API"""

    print("="*60)
    print(f"🚀 基金 Holdings -> Top 5 bond geographic API 捕获模式")
    ...
    print(f"\n🎯 基金 Holdings -> Top 5 bond geographic API 捕获完成！")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'screener':
        ...
        # 运行基金 Holdings -> Top 5 bond geographic 捕获
