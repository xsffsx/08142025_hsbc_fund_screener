...
def map_fields_to_api(frontend_fields: Dict[str, str], sample_product: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    # 获取Top 5 bond geographic数据
    bond_regional_data = sample_product.get("holdings", {}).get("bondRegional", {})
    ...
    if len(sorted_regions) > 0:
        ...
        first_api_path = f"holdings.bondRegional.bondRegionalExposures[name={sorted_regions[0]['api_key']}]"
    else:
        first_region_name = ""
        first_region_weight = ""
        first_api_path = "N/A"
    ...
    field_mapping["1st"] = {
        "frontend_name": "1st",
        "frontend_description": "第一大债券地理区域配置，包含债券地理区域名称和权重百分比",
    ...
    if len(sorted_regions) > 1:
        ...
        second_api_path = f"holdings.bondRegional.bondRegionalExposures[name={sorted_regions[1]['api_key']}]"
    else:
        second_region_name = ""
    ...
    field_mapping["2nd"] = {
        "frontend_name": "2nd",
        "frontend_description": "第二大债券地理区域配置，包含债券地理区域名称和权重百分比",
    ...
    if len(sorted_regions) > 2:
        ...
        third_api_path = f"holdings.bondRegional.bondRegionalExposures[name={sorted_regions[2]['api_key']}]"
    else:
        third_region_name = ""
        third_region_weight = ""
        third_api_path = "N/A"
    ...
    field_mapping["3rd"] = {
        "frontend_name": "3rd",
        "frontend_description": "第三大债券地理区域配置，包含债券地理区域名称和权重百分比",
    ...
    if len(sorted_regions) > 3:
        ...
        fourth_api_path = f"holdings.bondRegional.bondRegionalExposures[name={sorted_regions[3]['api_key']}]"
    else:
        fourth_region_name = ""
    ...
    field_mapping["4th"] = {
        "frontend_name": "4th",
        "frontend_description": "第四大债券地理区域配置，包含债券地理区域名称和权重百分比",
    ...
    if len(sorted_regions) > 4:
        ...
        fifth_api_path = f"holdings.bondRegional.bondRegionalExposures[name={sorted_regions[4]['api_key']}]"
    else:
        fifth_region_name = ""
        fifth_region_weight = ""
        fifth_api_path = "N/A"
    ...
    field_mapping["5th"] = {
        "frontend_name": "5th",
        "frontend_description": "第五大债券地理区域配置，包含债券地理区域名称和权重百分比",
...
**请求方法**: POST"""
    ...
    report = f"""# HSBC Fund Screener - Holdings -> Top 5 bond geographic 字段映射分析报告
...
"""
    ...
    for i, (field_key, mapping) in enumerate(field_mapping.items(), 1):
        ...
        field_descriptions = {
            ...
            '1st': '第一大债券地理区域配置，基金在债券投资中权重最高的地理区域',
            '2nd': '第二大债券地理区域配置，基金在债券投资中权重第二的地理区域',
            '3rd': '第三大债券地理区域配置，基金在债券投资中权重第三的地理区域',
            '4th': '第四大债券地理区域配置，基金在债券投资中权重第四的地理区域',
            '5th': '第五大债券地理区域配置，基金在债券投资中权重第五的地理区域'
        }

        field_remarks = {
            'Fund': '基金名称是用户识别产品的主要标识，通常包含基金公司、投资策略和份额类别信息',
            '1st': '第一大债券地理区域配置反映基金的核心债券投资地域策略，通常占债券投资的最大比重，是基金固定收益地域分布的重要体现',
            '2nd': '第二大债券地理区域配置显示基金的次要债券投资市场，与第一大区域形成投资组合的主要构成，有助于分散单一地域风险',
            '3rd': '第三大债券地理区域配置进一步丰富基金的债券地域分布，提供额外的投资机会和地域风险分散效果',
            '4th': '第四大债券地理区域配置增强基金的多元化债券投资特征，有助于捕捉不同地区的收益机会',
            '5th': '第五大债券地理区域配置完善基金的债券配置结构，展示基金在不同地理市场的投资布局和风险管理能力'
        }
        ...
        if 'api_value_2' in mapping and mapping['api_value_2']:
            # 构建API路径1和API路径2
            api_path_1 = f"{mapping['api_path']}.name"  # 债券地理区域名称路径
...
| **示例对比1** | {mapping['example']} | {api_example} | 债券地理区域名称 |
| **API路径1** | - | `{api_path_1}` | 债券地理区域名称的API路径 |
