...
class FieldExtractor:
    ...
    def _load_field_mapping(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "1st": {
                "api_path": "calculated:top_bond_sector[0]",
                "data_type": "string"
            },
            "2nd": {
                "api_path": "calculated:top_bond_sector[1]",
                "data_type": "string"
            },
            "3rd": {
                "api_path": "calculated:top_bond_sector[2]",
                "data_type": "string"
            },
            "4th": {
                "api_path": "calculated:top_bond_sector[3]",
                "data_type": "string"
            },
            "5th": {
                "api_path": "calculated:top_bond_sector[4]",
                "data_type": "string"
            }
        }
    ...
    def _get_nested_value(self, data: Dict[str, Any], path: str) -> Any:
        """从嵌套字典中获取值"""
        # 处理top bond sector排序字段：calculated:top_bond_sector[index]
        if path.startswith("calculated:top_bond_sector[") and path.endswith("]"):
            # 提取索引
            import re
            match = re.search(r'top_bond_sector\[(\d+)\]', path)
            if match:
                index = int(match.group(1))

                # 获取bondSectors数据并按权重排序
                holdings_data = data.get("holdings", {})
                bond_sector_data = holdings_data.get("bondSectors", {})
                ...
                # 按权重降序排序
                sorted_sectors = sorted(sectors, key=lambda x: x["weight"], reverse=True)

                # 返回指定索引的债券行业名称和权重百分比
                if index < len(sorted_sectors):
                    sector_name = sorted_sectors[index]["name"]
                    sector_weight = sorted_sectors[index]["weight"]
