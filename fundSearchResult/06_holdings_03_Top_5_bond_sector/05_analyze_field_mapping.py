...
def map_fields_to_api(frontend_fields: Dict[str, str], sample_product: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    # 获取Top 5 bond sector数据
    bond_sector_data = sample_product.get("holdings", {}).get("bondSectors", {})
    global_bond_sectors = bond_sector_data.get("globalBondSectors", [])

    # 创建债券行业权重列表，过滤掉Others和权重为0的项
    sectors = []
    bond_sector_mapping = {
        "GVT": "Government",
        "CORP": "Corporate",
    ...
    for sector in global_bond_sectors:
        sector_code = sector.get("name", "")
        weight = sector.get("weighting", 0)

        # 跳过Others和权重为0的项
        if sector_code != "Others" and weight > 0:
            display_name = bond_sector_mapping.get(sector_code, sector_code)
    ...
    if len(sorted_sectors) > 0:
        first_sector_name = sorted_sectors[0]["name"]
        first_sector_weight = sorted_sectors[0]["weight"]
        first_api_path = f"holdings.bondSectors.globalBondSectors[name={sorted_sectors[0]['api_key']}]"
    else:
        first_sector_name = ""
        first_sector_weight = ""
        first_api_path = "N/A"
    ...
    field_mapping["1st"] = {
        "frontend_name": "1st",
        "frontend_description": "第一大债券行业配置，包含债券行业名称和权重百分比",
        "api_path": first_api_path,
        "api_value": first_sector_name,
        "api_value_2": f"{first_sector_weight:.2f}%" if first_sector_weight else "",
        "data_type": "string",
        "example": first_sector_name if first_sector_name else "N/A",
        "example_2": f"{first_sector_weight:.2f}%" if first_sector_weight else "N/A"
    }

    # 2nd (第二大债券行业)
    if len(sorted_sectors) > 1:
        second_sector_name = sorted_sectors[1]["name"]
        second_sector_weight = sorted_sectors[1]["weight"]
        second_api_path = f"holdings.bondSectors.globalBondSectors[name={sorted_sectors[1]['api_key']}]"
    else:
        second_sector_name = ""
        second_sector_weight = ""
        second_api_path = "N/A"

    field_mapping["2nd"] = {
        "frontend_name": "2nd",
        "frontend_description": "第二大债券行业配置，包含债券行业名称和权重百分比",
        "api_path": second_api_path,
        "api_value": second_sector_name,
        "api_value_2": f"{second_sector_weight:.2f}%" if second_sector_weight else "",
        "data_type": "string",
        "example": second_sector_name if second_sector_name else "N/A",
        "example_2": f"{second_sector_weight:.2f}%" if second_sector_weight else "N/A"
    }

    # 3rd (第三大债券行业)
    if len(sorted_sectors) > 2:
        third_sector_name = sorted_sectors[2]["name"]
        third_sector_weight = sorted_sectors[2]["weight"]
        third_api_path = f"holdings.bondSectors.globalBondSectors[name={sorted_sectors[2]['api_key']}]"
    else:
        third_sector_name = ""
        third_sector_weight = ""
        third_api_path = "N/A"
    ...
    field_mapping["3rd"] = {
        "frontend_name": "3rd",
        "frontend_description": "第三大债券行业配置，包含债券行业名称和权重百分比",
        "api_path": third_api_path,
        "api_value": third_sector_name,
        "api_value_2": f"{third_sector_weight:.2f}%" if third_sector_weight else "",
        "data_type": "string",
        "example": third_sector_name if third_sector_name else "N/A",
        "example_2": f"{third_sector_weight:.2f}%" if third_sector_weight else "N/A"
    }

    # 4th (第四大债券行业)
    if len(sorted_sectors) > 3:
        fourth_sector_name = sorted_sectors[3]["name"]
        fourth_sector_weight = sorted_sectors[3]["weight"]
        fourth_api_path = f"holdings.bondSectors.globalBondSectors[name={sorted_sectors[3]['api_key']}]"
    else:
        fourth_sector_name = ""
        fourth_sector_weight = ""
        fourth_api_path = "N/A"

    field_mapping["4th"] = {
        "frontend_name": "4th",
        "frontend_description": "第四大债券行业配置，包含债券行业名称和权重百分比",
        "api_path": fourth_api_path,
        "api_value": fourth_sector_name,
        "api_value_2": f"{fourth_sector_weight:.2f}%" if fourth_sector_weight else "",
        "data_type": "string",
        "example": fourth_sector_name if fourth_sector_name else "N/A",
        "example_2": f"{fourth_sector_weight:.2f}%" if fourth_sector_weight else "N/A"
    }

    # 5th (第五大债券行业)
    if len(sorted_sectors) > 4:
        fifth_sector_name = sorted_sectors[4]["name"]
        fifth_sector_weight = sorted_sectors[4]["weight"]
        fifth_api_path = f"holdings.bondSectors.globalBondSectors[name={sorted_sectors[4]['api_key']}]"
    else:
        fifth_sector_name = ""
        fifth_sector_weight = ""
        fifth_api_path = "N/A"
    ...
    field_mapping["5th"] = {
        "frontend_name": "5th",
        "frontend_description": "第五大债券行业配置，包含债券行业名称和权重百分比",
        "api_path": fifth_api_path,
        "api_value": fifth_sector_name,
        "api_value_2": f"{fifth_sector_weight:.2f}%" if fifth_sector_weight else "",
        "data_type": "string",
        "example": fifth_sector_name if fifth_sector_name else "N/A",
        "example_2": f"{fifth_sector_weight:.2f}%" if fifth_sector_weight else "N/A"
    }
...
**请求方法**: POST"""
    ...
    report = f"""# HSBC Fund Screener - Holdings -> Top 5 bond sector 字段映射分析报告
...
"""
    ...
    for i, (field_key, mapping) in enumerate(field_mapping.items(), 1):
        ...
        field_descriptions = {
            ...
            '1st': '第一大债券行业配置，基金在债券投资中权重最高的行业',
            '2nd': '第二大债券行业配置，基金在债券投资中权重第二的行业',
            '3rd': '第三大债券行业配置，基金在债券投资中权重第三的行业',
            '4th': '第四大债券行业配置，基金在债券投资中权重第四的行业',
            '5th': '第五大债券行业配置，基金在债券投资中权重第五的行业'
        }

        field_remarks = {
            'Fund': '基金名称是用户识别产品的主要标识，通常包含基金公司、投资策略和份额类别信息',
            '1st': '第一大债券行业配置反映基金的核心债券投资策略，通常占债券投资的最大比重，是基金固定收益投资策略的重要体现',
            '2nd': '第二大债券行业配置显示基金的次要债券投资领域，与第一大行业形成投资组合的主要构成，有助于分散单一行业风险',
            '3rd': '第三大债券行业配置进一步丰富基金的债券行业分布，提供额外的投资机会和行业风险分散效果',
            '4th': '第四大债券行业配置增强基金的多元化债券投资特征，有助于捕捉不同行业的收益机会',
            '5th': '第五大债券行业配置完善基金的债券配置结构，展示基金在不同债券类型的投资布局和风险管理能力'
        }
        ...
        if 'api_value_2' in mapping and mapping['api_value_2']:
            # 构建API路径1和API路径2
            api_path_1 = f"{mapping['api_path']}.name"  # 债券行业名称路径
...
| **示例对比1** | {mapping['example']} | {api_example} | 债券行业名称 |
| **API路径1** | - | `{api_path_1}` | 债券行业名称的API路径 |
