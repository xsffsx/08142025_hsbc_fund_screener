"""
HSBC基金筛选器 - Holdings -> Top 5 equity sector 子标签页数据捕获脚本

功能：
1. 访问HSBC基金筛选页面，设置风险等级为"Speculative - 5"
2. 点击"Holdings"标签页
3. 保持"Top 5 equity sector"子标签（默认显示，无需点击）
4. 捕获页面数据和截图
5. 保存API响应数据到JSON文件
...
# Output paths relative to script location (06_holdings_01_Top_5_equity_sector directory)
SCRIPT_DIR = Path(__file__).resolve().parent
BASE_DIR = SCRIPT_DIR.parents[1]  # 项目根目录
DATA_DIR = BASE_DIR / f"data_{timestamp}"
DATA_DIR.mkdir(parents=True, exist_ok=True)

# 输出目录为脚本所在目录 (06_holdings_01_Top_5_equity_sector)
FUND_SEARCH_OUTPUT_DIR = SCRIPT_DIR
FUND_SEARCH_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# 子目录结构 - 在06_holdings_01_Top_5_equity_sector下创建
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                print(f"\n=== 步骤 4.1: 点击 Top 5 equity sector 子标签 ===")
                try:
                    # 查找并点击 "Top 5 equity sector" 子标签
                    # 根据您提供的HTML结构，使用li元素和文本匹配
                    sector_selector = 'li:has-text("Top 5 equity sector"), button:has-text("Top 5 equity sector"), a:has-text("Top 5 equity sector")'

                    # 等待 Top 5 equity sector 子标签出现
                    await page.wait_for_selector(sector_selector, timeout=10000)
                    print("✅ 找到 Top 5 equity sector 子标签")

                    # 点击 Top 5 equity sector 子标签
                    await page.click(sector_selector)
                    print("✅ 已点击 Top 5 equity sector 子标签")
                    log_debug("步骤4.1", "成功点击Top 5 equity sector子标签", {
                        "selector": sector_selector,
                        "action": "click_subtab",
                        "success": True
                    })
                    ...
                    # 等待子标签页面完全加载和新的API请求
                    await page.wait_for_timeout(3000)
                    print("✅ 等待Top 5 equity sector页面加载完成和新API请求")

                except Exception as e:
                    print(f"⚠️  点击 Top 5 equity sector 子标签失败: {e}")
                    log_debug("步骤4.1", "Top 5 equity sector子标签点击失败", {
                        "error": str(e),
                        "warning": True
                    })
                    print("⚠️  继续执行，使用原始数据")

                print(f"\n=== 步骤 4.2: 点击 Fund 列标题进行排序 ===")
                try:
                    # 查找并点击 "Fund" 列标题
                    # 根据您提供的HTML结构，使用data-locale-key属性
                    fund_selector = '[data-locale-key="OrderScreener.H_TABLE_FUND_CODE"], th:has-text("Fund"), button:has-text("Fund")'
                    ...
                    print(f"⚠️  点击 Fund 列标题失败: {e}")
                    log_debug("步骤4.2", "Fund列标题点击失败", {
                        "error": str(e),
                        "warning": True
                    })
                    print("⚠️  继续执行，使用原始数据")
...
async def main_fund_screener(headless: bool = True):
    """主函数：捕获基金 Holdings -> Top 5 equity sector 页面的 API"""

    print("="*60)
    print(f"🚀 基金 Holdings -> Top 5 equity sector API 捕获模式")
    ...
    print(f"\n🎯 基金 Holdings -> Top 5 equity sector API 捕获完成！")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'screener':
        ...
        # 运行基金 Holdings -> Top 5 equity sector 捕获
