...
class FieldExtractor:
    ...
    def _load_field_mapping(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "1st": {
                "api_path": "calculated:top_sector[0]",
                "data_type": "string"
            },
            "2nd": {
                "api_path": "calculated:top_sector[1]",
                "data_type": "string"
            },
            "3rd": {
                "api_path": "calculated:top_sector[2]",
                "data_type": "string"
            },
            "4th": {
                "api_path": "calculated:top_sector[3]",
                "data_type": "string"
            },
            "5th": {
                "api_path": "calculated:top_sector[4]",
                "data_type": "string"
            }
        }
    ...
    def _get_nested_value(self, data: Dict[str, Any], path: str) -> Any:
        """从嵌套字典中获取值"""
        # 处理top sector排序字段：calculated:top_sector[index]
        if path.startswith("calculated:top_sector[") and path.endswith("]"):
            # 提取索引
            import re
            match = re.search(r'top_sector\[(\d+)\]', path)
            if match:
                index = int(match.group(1))

                # 获取stockSectors数据并按权重排序
                holdings_data = data.get("holdings", {})
                stock_sectors_data = holdings_data.get("stockSectors", {})
                ...
                if index < len(sorted_sectors):
                    return sorted_sectors[index]["name"]
                return ""
