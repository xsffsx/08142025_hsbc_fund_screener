    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                # 步骤 4.1: 点击 5 year 子标签
                print(f"\n=== 步骤 4.1: 点击 5 year 子标签 ===")
                try:
                    # 查找并点击 "5 year" 子标签
                    five_year_selector = 'li:has-text("5 year"), button:has-text("5 year"), a:has-text("5 year"), [data-testid*="5-year"]'

                    # 等待 5 year 子标签出现
                    await page.wait_for_selector(five_year_selector, timeout=10000)
                    print("✅ 找到 5 year 子标签")

                    # 点击 5 year 子标签
                    await page.click(five_year_selector)
                    print("✅ 已点击 5 year 子标签")
                    log_debug("步骤4.1", "成功点击5 year子标签", {
                        "selector": five_year_selector,
                        "success": True
                    })
                    ...
                    print(f"❌ 点击 5 year 子标签失败: {e}")
                    log_debug("步骤4.1", "5 year子标签点击失败", {
                        "error": str(e),
                        "selector": five_year_selector
                    })
...
async def main_fund_screener(headless: bool = True):
    """主函数：捕获基金 Risk return profile -> 5 year 页面的 API"""

    print("="*60)
    print(f"🚀 基金 Risk Return Profile -> 5 year API 捕获模式")
    ...
    print(f"\n🎯 基金 Risk Return Profile -> 5 year API 捕获完成！")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'screener':
        ...
        # 运行基金 Risk Return Profile -> 5 year 捕获
