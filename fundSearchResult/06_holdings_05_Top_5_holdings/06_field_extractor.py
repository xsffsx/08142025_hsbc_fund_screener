...
class FieldExtractor:
    ...
    def _load_field_mapping(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "1st": {
                "api_path": "calculated:top_holdings[0]",
                "data_type": "string"
            },
            "2nd": {
                "api_path": "calculated:top_holdings[1]",
                "data_type": "string"
            },
            "3rd": {
                "api_path": "calculated:top_holdings[2]",
                "data_type": "string"
            },
            "4th": {
                "api_path": "calculated:top_holdings[3]",
                "data_type": "string"
            },
            "5th": {
                "api_path": "calculated:top_holdings[4]",
                "data_type": "string"
            }
        }
    ...
    def _get_nested_value(self, data: Dict[str, Any], path: str) -> Any:
        """从嵌套字典中获取值"""
        # 处理top holdings排序字段：calculated:top_holdings[index]
        if path.startswith("calculated:top_holdings[") and path.endswith("]"):
            # 提取索引
            import re
            match = re.search(r'top_holdings\[(\d+)\]', path)
            if match:
                ...
                # 返回指定索引的持仓名称和权重百分比
                if index < len(sorted_holdings):
                    holding_name = sorted_holdings[index]["name"]
                    holding_weight = sorted_holdings[index]["weight"]
                    return f"{holding_name}\n{holding_weight:.2f}%"
                return ""
