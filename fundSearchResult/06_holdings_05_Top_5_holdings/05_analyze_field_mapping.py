...
def map_fields_to_api(frontend_fields: Dict[str, str], sample_product: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    # 获取Top 5 holdings数据
    holdings_data = sample_product.get("holdings", {})
    top_holdings_list = holdings_data.get("topHoldingsList", [])

    # 创建前5大持仓列表，过滤掉Others和权重为0的项
    holdings = []

    for holding in top_holdings_list:
        holding_name = holding.get("holdingName", "")
        holding_company = holding.get("holdingCompany", "")
        holding_percent = holding.get("holdingPercent", 0)

        # 跳过Others和权重为0的项，只取前5个
        if holding_company != "Others" and holding_percent > 0 and len(holdings) < 5:
            holdings.append({
                "name": holding_company,
                "weight": holding_percent,
                "rank": holding_name
            })

    # 前5大持仓已经按权重排序
    sorted_holdings = holdings[:5]

    # 1st (第一大持仓)
    if len(sorted_holdings) > 0:
        first_holding_name = sorted_holdings[0]["name"]
        first_holding_weight = sorted_holdings[0]["weight"]
        first_api_path = f"holdings.topHoldingsList[{sorted_holdings[0]['rank']}]"
    else:
        first_holding_name = ""
        first_holding_weight = ""
        first_api_path = "N/A"

    field_mapping["1st"] = {
        "frontend_name": "1st",
        "frontend_description": "第一大持仓配置，包含持仓名称和权重百分比",
        "api_path": first_api_path,
        "api_value": first_holding_name,
        "api_value_2": f"{first_holding_weight:.2f}%" if first_holding_weight else "",
        "data_type": "string",
        "example": first_holding_name if first_holding_name else "N/A",
        "example_2": f"{first_holding_weight:.2f}%" if first_holding_weight else "N/A"
    }

    # 2nd (第二大持仓)
    if len(sorted_holdings) > 1:
        second_holding_name = sorted_holdings[1]["name"]
        second_holding_weight = sorted_holdings[1]["weight"]
        second_api_path = f"holdings.topHoldingsList[{sorted_holdings[1]['rank']}]"
    else:
        second_holding_name = ""
        second_holding_weight = ""
        second_api_path = "N/A"

    field_mapping["2nd"] = {
        "frontend_name": "2nd",
        "frontend_description": "第二大持仓配置，包含持仓名称和权重百分比",
        "api_path": second_api_path,
        "api_value": second_holding_name,
        "api_value_2": f"{second_holding_weight:.2f}%" if second_holding_weight else "",
        "data_type": "string",
        "example": second_holding_name if second_holding_name else "N/A",
        "example_2": f"{second_holding_weight:.2f}%" if second_holding_weight else "N/A"
    }

    # 3rd (第三大持仓)
    if len(sorted_holdings) > 2:
        third_holding_name = sorted_holdings[2]["name"]
        third_holding_weight = sorted_holdings[2]["weight"]
        third_api_path = f"holdings.topHoldingsList[{sorted_holdings[2]['rank']}]"
    else:
        third_holding_name = ""
        third_holding_weight = ""
        third_api_path = "N/A"

    field_mapping["3rd"] = {
        "frontend_name": "3rd",
        "frontend_description": "第三大持仓配置，包含持仓名称和权重百分比",
        "api_path": third_api_path,
        "api_value": third_holding_name,
        "api_value_2": f"{third_holding_weight:.2f}%" if third_holding_weight else "",
        "data_type": "string",
        "example": third_holding_name if third_holding_name else "N/A",
        "example_2": f"{third_holding_weight:.2f}%" if third_holding_weight else "N/A"
    }

    # 4th (第四大持仓)
    if len(sorted_holdings) > 3:
        fourth_holding_name = sorted_holdings[3]["name"]
        fourth_holding_weight = sorted_holdings[3]["weight"]
        fourth_api_path = f"holdings.topHoldingsList[{sorted_holdings[3]['rank']}]"
    else:
        fourth_holding_name = ""
        fourth_holding_weight = ""
        fourth_api_path = "N/A"

    field_mapping["4th"] = {
        "frontend_name": "4th",
        "frontend_description": "第四大持仓配置，包含持仓名称和权重百分比",
        "api_path": fourth_api_path,
        "api_value": fourth_holding_name,
        "api_value_2": f"{fourth_holding_weight:.2f}%" if fourth_holding_weight else "",
        "data_type": "string",
        "example": fourth_holding_name if fourth_holding_name else "N/A",
        "example_2": f"{fourth_holding_weight:.2f}%" if fourth_holding_weight else "N/A"
    }

    # 5th (第五大持仓)
    if len(sorted_holdings) > 4:
        fifth_holding_name = sorted_holdings[4]["name"]
        fifth_holding_weight = sorted_holdings[4]["weight"]
        fifth_api_path = f"holdings.topHoldingsList[{sorted_holdings[4]['rank']}]"
    else:
        fifth_holding_name = ""
        fifth_holding_weight = ""
        fifth_api_path = "N/A"

    field_mapping["5th"] = {
        "frontend_name": "5th",
        "frontend_description": "第五大持仓配置，包含持仓名称和权重百分比",
        "api_path": fifth_api_path,
        "api_value": fifth_holding_name,
        "api_value_2": f"{fifth_holding_weight:.2f}%" if fifth_holding_weight else "",
        "data_type": "string",
        "example": fifth_holding_name if fifth_holding_name else "N/A",
        "example_2": f"{fifth_holding_weight:.2f}%" if fifth_holding_weight else "N/A"
    }
...
**请求方法**: POST"""
    ...
    report = f"""# HSBC Fund Screener - Holdings -> Top 5 holdings 字段映射分析报告
...
"""
    ...
    for i, (field_key, mapping) in enumerate(field_mapping.items(), 1):
        ...
        field_descriptions = {
            ...
            '1st': '第一大持仓配置，基金投资组合中权重最高的单一持仓',
            '2nd': '第二大持仓配置，基金投资组合中权重第二的单一持仓',
            '3rd': '第三大持仓配置，基金投资组合中权重第三的单一持仓',
            '4th': '第四大持仓配置，基金投资组合中权重第四的单一持仓',
            '5th': '第五大持仓配置，基金投资组合中权重第五的单一持仓'
        }

        field_remarks = {
            'Fund': '基金名称是用户识别产品的主要标识，通常包含基金公司、投资策略和份额类别信息',
            '1st': '第一大持仓反映基金的核心投资策略，通常占投资组合的最大比重，是基金投资风格和重点配置的重要体现',
            '2nd': '第二大持仓显示基金的次要投资重点，与第一大持仓形成投资组合的主要构成，有助于分散单一持仓风险',
            '3rd': '第三大持仓进一步丰富基金的投资分布，提供额外的投资机会和风险分散效果',
            '4th': '第四大持仓增强基金的多元化投资特征，有助于捕捉不同投资标的的收益机会',
            '5th': '第五大持仓完善基金的投资配置结构，展示基金在不同投资标的的布局和风险管理能力'
        }
        ...
        if 'api_value_2' in mapping and mapping['api_value_2']:
            # 构建API路径1和API路径2
            api_path_1 = f"{mapping['api_path']}.holdingCompany"  # 持仓名称路径
            api_path_2 = f"{mapping['api_path']}.holdingPercent"  # 权重百分比路径
...
| **示例对比1** | {mapping['example']} | {api_example} | 持仓名称 |
| **API路径1** | - | `{api_path_1}` | 持仓名称的API路径 |
