...
|----|---------|--------|---------|-------------|---------------|-------------|-----------|----------|----------|
| 103 | **Fund code / Keywords** | Free text input | `"BlackRock"` | `request.searchText` | Multiple fields | Multiple tables | Multiple columns | ⚠️ ELK搜索 | ✅  已验证 |
| 104 | **Asset class** | Asia Pacific Ex-Japan Equity - Broad | `"AB"` | `listCriterias[criteriaKey=CAT].items[itemKey=AB]` | `utProd.categoryCode` | V_UT_PROD_INSTM | `FUND_CAT_CDE` | 代码映射 | ✅ 已验证 |
| 105 | **Geography** | Global | `"GL"` | `listCriterias[criteriaKey=INVSTRG].items[itemKey=GL]` | `investmentRegionCode` | V_UT_PROD_INSTM | `MKT_INVST_CDE` | 代码映射 | ✅ 已验证 |
| 106 | **Fund house** | BlackRock | `"MERRL"` | `listCriterias[criteriaKey=FAM].items[itemKey=MERRL]` | `utProd.familyCode` | V_UT_PROD_INSTM | `FUND_FM_CDE` | 代码映射 | ✅ 已验证 |
| 107 | **By your risk tolerance** | Speculative - 5 | `"5"` | `listCriterias[criteriaKey=RISK].items[itemKey=5]` | `utProd.riskLvlCde` | V_UT_PROD_INSTM | `RISK_LVL_CDE` | 直接映射 | ✅ 已验证 |
| 108 | **Fund currency** | USD | `"USD"` | `listCriterias[criteriaKey=CCY].items[itemKey=USD]` | `utProd.currency` | V_UT_PROD_INSTM | `CCY_PROD_CDE` | 直接映射 | ✅ 已验证 |
| 109 | **GBA Wealth Connect** | ✓ Selected | `"Y"` | `listCriterias[criteriaKey=GBAA].items[itemKey=Y]` | `utProd.gbaAcctTrdb` | V_UT_PROD_INSTM | `GBA_ACCT_TRDB` | 标志映射 | ✅ 已验证 |
| 110 | **ESG investments** | ✓ Selected | `"Y"` | `listCriterias[criteriaKey=ESG].items[itemKey=Y]` | `utProd.esgInd` | V_UT_PROD_INSTM | `ESG_IND` | 标志映射 | ✅ 已验证 |
| 111 | **1 year annualised return** | Range: -51.05% to 133.35% | `{minimum: -51.04129, maximum: 133.34676}` | `minMaxCriterias[criteriaKey=Y1RTRN]` | `return1yrDaily` | V_UT_PROD_INSTM | `RTRN_1YR_DPN` | 范围聚合 | ✅ 已验证 |
| 112 | **Dividend yield** | Range: 0.00% to 10.11% | `{minimum: 0, maximum: 10.10726}` | `minMaxCriterias[criteriaKey=DIVYLD]` | `yield1Yr` | V_UT_PROD_INSTM | `YIELD_1YR_PCT` | 范围聚合 | ✅ 已验证 |
| 113 | **Morningstar rating** | ★★★★★ | `"5"` | `listCriterias[criteriaKey=MSTAR].items[itemKey=5]` | `ratingOverall` | V_UT_PROD_INSTM | `PROD_OVRL_RATENG_NUM` | 数值转星级 | ✅ 已验证 |
| 114 | **Average credit quality** | AAA | `"AAA"` | `listCriterias[criteriaKey=ACQN].items[itemKey=AAA]` | `averageCreditQualityName` | V_UT_PROD_INSTM | `AVG_CR_QLTY_NAME` | 直接映射 | ✅ 已验证 |
| 115 | **1 year quartile ranking** | 1st | `"1"` | `listCriterias[criteriaKey=Y1QTL].items[itemKey=1]` | `rank1Yr` | V_UT_PROD_INSTM | `RANK_QTL_1YR_NUM` | 数值转序数 | ✅ 已验证 |
