...
async def capture_fund_screener_api(headless: bool = True, save_to_dir: Path = None) -> Dict[str, Any]:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print(f"\n=== 步骤 4: 点击 Fees and charges 标签 ===")
            # 等待页面加载完成，确保标签页可见
            await page.wait_for_timeout(2000)

            try:
                # 查找并点击 "Fees and charges" 标签
                fees_selector = 'button:has-text("Fees and charges"), a:has-text("Fees and charges"), [data-testid*="fees"], [data-testid*="charges"]'

                # 等待 Fees and charges 标签出现
                await page.wait_for_selector(fees_selector, timeout=10000)
                print("✅ 找到 Fees and charges 标签")

                # 点击 Fees and charges 标签
                await page.click(fees_selector)
                print("✅ 已点击 Fees and charges 标签")
                log_debug("步骤4", "成功点击Fees and charges标签", {
                    "selector": fees_selector,
                    "success": True
                })
                ...
                # 验证是否成功切换到 Fees and charges 页面
                current_url = page.url
                print(f"✅ Fees and charges 页面URL: {current_url}")

                # 确认点击成功 - 检查页面是否包含 Fees and charges 相关内容
                fees_content = await page.query_selector('[data-testid*="fees"], [data-testid*="charges"], .fees, .charges, #fees')
                if fees_content or "fees" in current_url.lower() or "charges" in current_url.lower():
                    print("✅ 确认成功切换到 Fees and charges 页面")
                    log_debug("步骤4", "Fees and charges页面切换成功", {
                        "url": current_url,
                        "content_found": fees_content is not None,
                        "success": True
                    })
                else:
                    print("⚠️  无法确认是否成功切换到 Fees and charges 页面，继续执行...")
                    log_debug("步骤4", "Fees and charges页面切换状态不明", {
                        "url": current_url,
                        "content_found": False,
                        "warning": True
                    })

                # 等待Fees and charges页面完全加载
                await page.wait_for_timeout(3000)

                print(f"\n=== 步骤 4.1: 点击 Annual management fee (maximum) 列标题 ===")
                try:
                    # 查找并点击 "Annual management fee (maximum)" 列标题
                    fee_selector = 'th:has-text("Annual management fee"), button:has-text("Annual management fee"), [data-testid*="annual"], [data-testid*="management"], [data-testid*="fee"]'

                    # 等待 Annual management fee 列标题出现
                    await page.wait_for_selector(fee_selector, timeout=10000)
                    print("✅ 找到 Annual management fee (maximum) 列标题")

                    # 点击 Annual management fee 列标题进行排序
                    await page.click(fee_selector)
                    print("✅ 已点击 Annual management fee (maximum) 列标题")
                    log_debug("步骤4.1", "成功点击Annual management fee列标题", {
                        "selector": fee_selector,
                        "success": True
                    })
                    ...
                    print(f"⚠️  点击 Annual management fee (maximum) 列标题失败: {e}")
                    log_debug("步骤4.1", "Annual management fee列标题点击失败", {
                        "error": str(e),
                        "warning": True
                    })
                    print("⚠️  继续执行，使用原始数据")
                ...
                print(f"❌ 点击 Fees and charges 标签失败: {e}")
                log_debug("步骤4", "Fees and charges标签点击失败", {
                    "error": str(e),
                    "selector": fees_selector
                })
                print("⚠️  继续执行，但可能无法获取 Fees and charges 数据")
...
async def main_fund_screener(headless: bool = True):
    """主函数：捕获基金 Fees and charges 页面的 API"""

    print("="*60)
    print(f"🚀 基金 Fees and charges API 捕获模式")
